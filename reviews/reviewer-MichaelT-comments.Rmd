...
## Chapter 3 June 2025 Edition

I have a bad feeling of regarding the `::: {.panel-tabset}` in this chapter--I don't think the pdf will respond well to it...
...
[MF: that's an issue with the print method. :(]
...
- I think it would be very helpful to describe what the null hypothesis for the tests associated with Figure 9.2 are. I understand that in Section 9.1 you gave an example null hypothesis, but I presume that's different from the null hypothesis here [DONE]

- On further reflection, perhaps we could shuffle the content between chapters? Discussion around $T^2$ requires discussion of multivariate linear models. But the motivation behind multivariate linear models and some of its visualizations requires both chapter 9 and 11...


### 9.3

I love using the R code to illustrate how the data can be related to the HE plot more closely

### 9.7

- When is canonical space discussed? It shows in the figure summarizing the chapter, but I don't think ti's been discussed that much this chapter..?
...
# Chapter 10 - May 2025 Edition

## Section 10.1.1

- Why do some assumptions start with '*'? Are the '*' meant to denote different assumptions?

## Other Sections

- I don't know if it's because I know more now, but somehow everything made sense when I looked at Figure 10.2. Also, reviewing Chapter 1: Introduction also made value multivariate linear models more. Modelling the correlations between the response variables could be important in something like behavioral memory research. Take https://psycnet.apa.org/record/2002-06812-014 for example. There are multiple response variables, yet the correlation amongst the various response variables is not considered.

- For consistency/parallels in writing, I think the text elaborating the different panels for Figure 10.2 should also explicitly refer to $SSP_H$ and $SSP_E$, as opposed to referring to the middle panel and the right panel. [**DONE**]

## Section 10.3.1

I'm not sure I understand, but isn't the $C$ matrix shown here getting $\beta_1$ and $\beta_2$, and not $\beta_2$ and $\beta_3$? I'm not sure what the example $CB$ has to do with... testing whether $x_2$ and $x_3$ contribute? [**DONE**]

- The discussion around contrasts for dogfood makes a lot more sense to me now.

> For example, Ours vs. Theirs estimated by formulac1 takes 0.69 less time to start eating and eats 5.44 more on average.

- Why did you divide the estimated $\beta$ coefficients by two? I thought this should already handled because your contrast for fomulac1 divides things by two?

## ANOVA --> MANOVA

- Regarding Figure 10.3 -- perhaps the issues could be made blindingly clear to the reader by including 1D lineplots? Could have a 1D horizontal lineplot for math, then a 1D vertical lineplot for reading. [Too hard to do]

- This chapter could easily be split into multiple chapters. [**How / where to split?**]

- For figure 10.3 and 10.4, is there a way to verbally describe what's happening with numbers? For example, the plot of the RHS of Figure 10.4 shows how the bivariate ellipse differs between the three groups. But what about verbally stating this difference? Or verbally describing the characteristics of the ellipse alongside the uncertainty in our statements?
...
mod.mlm <- brm(mvbind(caring, play, emotion) ~ group, data = Parenting,
...
Is there a quantitative write-up that we can provide for this? I understand that graphs are very helpful for understanding, but that the accompanying write-up with specific numbers in mind is very important for using this technique in live research and dissemination.

> How do anxiety and depression vary jointly with grade?

Similarly, some code demonstrating how this could be answered would be appreciated
[This is described in the example]

## MRA --> MMRA

Section 10.5: - Presumably, MRA = multiple linear regressio nand MMRA = multivariate multiple linear regression, a brief comment regarding this would be helpul.

## Model Diagnostics for MLMs
...
I think it would be very illuminating to also include the equation for an ellipsoid, then to show how the statistical output of a MLM is used to construct both the H and the E ellipsoid.


### Section 11.2

- I am confused--what are the differences between $\mathcal{E}_c, \mat{E}, \text{ and } \mathcal{E}_c$?

- I feel that the text in this first part before section 11.2.1 could be better understood through some diagrams for each component... It's hard for me to track how everything is related to each other

#### Section 11.2.1

I think `GGally::ggpairs()` could be nice here? Is the ggplot2 ellipse no good? https://ggplot2.tidyverse.org/reference/stat_ellipse.html ?
[I prefer car::scatterplotMatrix]

I'm curious about how much work it would be to add ggplot2 functionality to the `heplots` package? If there's a good separation between the data structure for the plot and the plot itself, it might not be too bad... One thing is the tidy data structure that ggplot2 requires...

[This is a project for the future.]

### 11.3

~~I'm not sure why I don't know this, but why does the error ellipse seem to have positive covariance in Figure 11.4? What determines its thickness and orientation?~~

- This is answered right-away, great!


### 11.5

- Is Figure 11.6 effect or significance scaled?

- Is there a way to do a significance test of how the correlation between the response variables changes based on the predictor variables?

- Relatedly, how would you do a HE plot of how the difference in the correlations is statistically significant or not?

[I don't quite understand these questions. With a factor, there are within-group and between group correlations]

### 11.7 

I think opening with a HE pairs plot of the correlations between response variables for each species would help motivate this section. Otherwise, I feel like the motivation is a bit lost on the reader. I understand that you state this textually, "However in a one-way MANOVA design with more than response three variables, it is difficult to visualize how the groups vary on all responses together, and how the different variables contribute to discrimination among groups." -- however, without the graph, I feel that it's all a bit too abstract to me. 

I think explaining more about canonical analysis and what it has to do with our problem of visualizing differences amongst all response varibles would help.

~~Also, what happens when you have multiple sets of predictors and a moderation effect?~~
- I see this is explained in the section on MMRA and MANCOVA

### 11.7.1

It seems like the math for the weighted sum of the canonical variate needs to be put in latex form [done]

- I don't think the term 'observation scores' has been explained yet?
- Similarly, I don't think 'canonical scores' have been explained yet [done]

### 11.8 

I think the section title could be renamed to also reflect how MANCOVA can do a mixture of factors and covariates [??]

> The overall joint test for both predictors can then visualized as the test of the linear hypothesis

to

> The overall joint test for both predictors can then be visualized as the test of the linear hypothesis [**DONE**]

### Canonical Correlation Analysis

- I think it's a bit confusing whether your discussion of whether X and Y are symmetric or not, refers to MANOVA or canonical correlation analysis. I think it's implied that they're different, but I'm not sure?

- I understand the explanation regarding how distinct pairs of canonical variables are orthogonal to each other, but perhaps to make it more accessible, we could sprinkle in a running concrete example?


### 11.9.1

Could you please elaborate on how the orientation of education's effect in Figure 11.13 tells us that education is more related to reading than mathematics?

### 11.9.2

Could you please elaborate on how the `98.6%' for the first canonical variate relates to the R output?

Could you also please elaborate on how the canonical analysis relates to the substantive aspects of the school data?
...
- Typo: covariance [fixed]

$$
M = (N -g) \ln \;|\; \mathbf{S}_p \;|\; - \sum_{i=1}^g (n_i -1) \ln \;|\; \mathbf{S}_i \;|\; \; ,
$$ {eq-boxm}

- Dangling {eq-boxm} ? [fixed]

## 12.4 Box's M test

- I think more clarification regarding what each of the variables are could help. Is $g$ the number of groups? What is $p$?
...
- I assume that by liberal you mean that the p-value approaches 0. Perhaps that could be clarified? I find it confusing when I read big or small $p$ values, it seems a bit ambiguous to me... [now refer to actual vs nominal $\alpha$]
...
- Is there a way we could make the color and shapes for Schizo and SchizAff more similar to each other than to Control? Would this be a good idea?
...
Had one of the tests been sig and the others were non-sig, would you conclude the same thing?

## Figure 13.5

- Is there a way we could fill the upper triangal or diagonal with other kinds of plots? What might be a nice addition?

## General Review

- I quite appreciate this example and I wishthere were more extensive examples like this throughout the book. As an overall review, I somewhat wonder if datasets on recent trendy topics could be analyzed, as this might spur the reader's imagination more

# Attempts to get Renv to do something

```{r, eval = FALSE}
pak::pak(c("url::https://cran.r-project.org/src/contrib/Archive/tourrGui/tourrGui_0.4.tar.gz", "url::https://cran.r-project.org/src/contrib/Archive/RGtk2/RGtk2_2.20.36.3.tar.gz", "url::https://cran.r-project.org/src/contrib/Archive/gWidgets/gWidgets_0.0-54.2.tar.gz"))
# TODO: RGtk2 is broken apparently, maybe not supported?
```
