...
- **Other statistics**: Box's $M$-test is most widely used. Are there other worthwhile test statistics? We will see that graphics methods suggest alternatives in @sec-other-measures.
...
The main methods described here are implemented in the `r package("heplots")`.
...
## Multivariate analog of Levine's test
...
(which has not been well-studied)[^Anderson] ---Simply do a MANOVA of the absolute differences,
$\vert \mathbf{Y} - \overline{\mathbf{Y}}_j \vert$, centering on the means or medians.
For the `iris` data, this gives:

[^Anderson]: @Anderson2006 PERMDISP method
...
The `irisdev.mod` model is just another MANOVA, so we can visualize it in an HE plot:

```{r}
#| label: fig-iris-dev-pairs
#| fig-width: 8
#| fig.height: 8
#| fig-cap: "HE plots of the MLM for the multivariate generalization of Levine's test."
