...
- **Visualization**: How can we visualize differences among group variances and covariance matrices,
  ...
  As will be illustrated (@sec-viz-boxM), differences among covariance matrices can be comprised of spread in overall size ("scatter") and shape ("orientation"). These can be seen in data space with data ellipses, particularly if the data is centered by shifting all groups to the grand mean.
...
- **Other statistics**: Box's $\mathcal{M}$-test is most widely used. Are there other worthwhile test statistics? You will see that graphical methods suggest alternatives in @sec-other-measures. Similarly, the univariate
Levene's test has a simple multivariate generalization which can be visualized using HE plots (@sec-multivar-levene).

The following sections provide a capsule summary of the issues in this topic.
...
But what does Box's $\mathcal{M}$ test (described below) say? As you can see,
...
putting numbers to the visual conslusion from @fig-iris-covEllipse-pairs.
:::
...
(a) a simple dotplot of the components of Box's $\mathcal{M}$ test: the log determinants of the 
...
Some other generalizations of Box's test using other measures are illustrated
...
#| fig-width: 12
#| out-width: "100%"
#| fig-cap: "Covariance ellipsoids for the first two principal components of the iris data. Left (a): uncentered, showing group means on the principal components; right (b): centered at the origin."
...
But you must remember that this is showing differences among groups in multivariate dispersion rather
than in means.
...
#| fig-width: 8
#| fig.height: 8
#| fig-cap: "HE plots for the multivariate generalization of Levine's test based on MANOVA of absolute deviations from group medians."
pairs(irisdev.mod, variables=1:4, fill=TRUE, fill.alpha=.1)
```

In this plot, the $\mathbf{H}$ ellipses for the groups show the variation in the overall size of absolute differences
...
the the Levene-type MANOVA in 1D or 2D. For the model `irisdev.mod`, this shows that 98% of groups differences
shown in the HE plots (@fig-iris-dev-pairs) can be found in a single canonical dimension.
...
Plotting the first canonical dimension gives boxplots for the scores and vectors for the weights of the
iris variables. Overall, the covariance matrices are smallest for setosa and largest for virginica.
...
#| fig.height: 5
#| out-width: "100%"
#| fig-cap: "Canonical discriminant plot for the multivariate generalization of Levine's test based on MANOVA of absolute deviations from group medians."
