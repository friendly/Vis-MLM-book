...
**TODO**: Add a formula method to `leveneTests()`

More conveniently, `heplots:leveneTests()` with an "s", does this for each of a set of response variables. It also formats the results
...
So, this tells us that the groups do not differ in variances on all variables except for `body_mass`.
...
For the Penguin data, it might be simplest just boxplots of the variables and try to see whether the widths of the
...
Instead, we can see more directly what is tested by the Levene test by graphing the absolute deviations from the group means or medians,
...
$$
This is much stronger than in the univariate case, because it also required that all the correlations between pairs of variables are the same for all
groups. ...

**Insert** formulas for 2 x 2 case
