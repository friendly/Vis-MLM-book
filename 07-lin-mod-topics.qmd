...
For linear models, particularly regression models with quantitative predictors, there is another space---"$\mathbf{\beta}$ space"---that provides deeper views of models and the relationships among them. This discussion follows @Friendly-etal:ellipses:2013, Sec. 4.6.
...
$(\bar{x}_1, \bar{x}_2)^\textsf{T} \oplus c \sqrt{\mathbf{S}_{\mathbf{X}}}$, where the covariance matrix $\mathbf{S}_{\mathbf{X}}$ depends on $\mathbf{X}\trans \mathbf{X}$.
...
It is well to understand the underlying geometry here connecting the ellipses for
...
are identical, while the eigenvalue are $1 / \lambda_i$, so the radii are
$1 / \sqrt{\lambda_i}$; the ellipse for $(2 \mathbf{S}^{-1})$ is smaller
...
*  Any _joint_ hypothesis (e.g., $H_0:\beta_{\mathrm{Stress}}=0, \beta_{\mathrm{Coffee}}=0$)
...
which is scaled by $\sqrt{F^{.95}_{1, \nu}} = t^{.975}_\nu$. We call this the _confidence-interval generating ellipse" (or, more compactly, the "confidence-interval ellipse").
