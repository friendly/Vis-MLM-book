...
of coffee and stress in data space and beta space.
The left panel in @fig-coffee-data-beta-both 
is the same as that in the (3,2) cell of @fig-coffee-scatmat for the relation `Stress ~ Coffee`.
The right panel shows the joint 95% confidence region for the coefficients $(\beta_{\text{Coffee}}, \beta_{\text{Stress}})$
in $\mathbf{\beta}$ space.  
...
#| echo: false
#| out-width: "100%"
#| fig-cap: "Data space and $\\mathbf{\\beta}$ space representations of Coffee and Stress.  Left: Standard (68%) data ellipse. Right: Joint 95% confidence ellipse (blue) for ($\\beta_{\\text{Coffee}}, \\beta_{\\text{Stress}}$), CI ellipse (red) with 95% univariate shadows"
