...
in a simple linear regression. Points in this space are models (true, hypothesized, fitted) whose coordinates represent values of these parameters. For example, in simple regression, one point plotted with the coefficients
...
would give weighted least squares and maximum likelihood estimates. The line
...
In the sense described below, the data space of $\mathbf{X}$ and that of $\boldsymbol{\beta}$ space are each _dual_  and _inverse_ to the other.
...
based on $\mathbf{S}_X^{-1}$. This is shown in @fig-coffee-data-beta-both.
...
### Coffee data: Bias and precision $\beta$ space
