...
with a mean vector $\boldsymbol{\mu}$ and variance-covariance matrix $\boldsymbol{\Sigma}$,
$$
...
This also provides a simple method to identify potential outliers as those points which are furthest
from the centroid; that is those 
for which
$D^2_{(i)}$ is greater than the $1 - \alpha$ quantile of $\chi^2_p$.


[^quantile]:  
The $p$-quantile of a random variable $Y$ is a value, denoted by $Q_Y(p)$, such that
...
You can illustrate the calculation of $D^2_{(i)}$ using `heplots::Mahalanobis()` for the
...
about 9 to have larger squared distances than this cutoff.
...
with the greatest $D^2_{(i)}$ values to be identified with their row numbers in the data.
```{r}
...
The confidence envelope is calculated by finding the standard errors of the $D^2_{(i)}$,

$$
 \text{se} ( D^2_{(i)} ) = \hat{b} /d ( q_i)) \times \sqrt{  p_i (1-p_i) / n }  \comma
$$
where  
$\hat{b}$ is an estimate of the slope of
...
The fact that larger values fall beneath the reference line indicates that the sample $D^2$ are
somewhat _less concentrated_ than values in the $\chi^2_2$ distribution.
...
and made distinctive in the plot.
