...
-   Data beyond 50 years is thin, but there are some quite low salaries
    ...
    marginal distributios of the observations. Jitter and transparency helps to avoid overplotting
...
#| out-width: "80%"
#| code-fold: show
#| fig-cap: !expr glue("Scatterplot of Salary vs. years since PhD, adding the loess smooth. The loess smooth curve and confidence band in {green} is nearly indistinguishable from a quadratic fit in {blue}.)
...
A contour of constant Euclidean distance, shown by the `{r} red` dashed circle,
ignores the apparent negative correlation, so point "A" is further.
The `{r} blue` ellipse for Mahalanobis distance 
...
* The lengths of arms of the `{r} blue` dashed central cross 
...
on average 
`coef(lm(prestige ~ education, data=Prestige))["income"]` =
`r coef(lm(prestige ~ education, data=Prestige))["income"]` with each year of education.
