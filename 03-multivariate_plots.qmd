...
*  `continuous`: both X and Y are continuous variables; supply as the `NAME` part of a `ggally_NAME()` function or name of a custom function.
*  `combo`: one X of and Y variable is discrete while the other is continuous;
...
in the lower triangle. The diagonal panels as shown as density plots (`ggally_densityDiag()`)
for continuous variables but as par plots (`ggally_densityDiag()`).
...
For this example, I want to summarize 
...
4 x 4 block of plots in the upper left. ...
...
extend data visualization to multivariate data, but these variables share one 2D space, and resolution decreases as the number of variable increase.
