...
* **The `lm()` function is a tool for thinking**: Whether you're fitting simple regression, multiple regression,
ANOVA, or ANCOVA models, `lm()` provides a unified interface through R's formula syntax.
A formula like `y ~ x1 + x2 + x1:x2` captures complex relationships with compact, intuitive notation.
...
components with functions like `coef()`, `fitted()`, `residuals()`, and `model.matrix()` makes the full contents of a fitted model accessible.
...
Seeing how R translates a formula like `y ~ treatment + block` into a matrix of 0s and 1s clarifies how categorical predictors
