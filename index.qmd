...
* **bold** : is used for names of R packages. 
  ...
  Perhaps I can use  "r colorize("**lattice**", "darkgreen")" inline -> `r colorize("**lattice**", "darkgreen")` will do this? This does bold & color, but can't use monospace.
  
  I can now use inline 'pkg("lattice")' generating `r pkg("lattice")`, or also with a citation,
  `pkg("lattice", cite=TRUE)` -> `r pkg("lattice", cite=TRUE)`, but can't do color or monospace with this.
