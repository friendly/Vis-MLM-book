...
The single symbol $\mathbf{X}$ can stand for an entire data table and the notation $\mathbf{y} = $\mathbf{X} \boldsymbol{\beta}$
represents an entire linear model.
...
For those not familiar with R, I recommend @Matloff-2011, @Wickham2014, and @Cotton-2013 for introductions on programming in the language. @FoxWeisberg:2018 and @Teetor2011 are great for learning about how to conduct basic statistical analyses in R.
**TODO**: Revise this list.

## Recommended R graphics books

In this book, I ...

* Claus Wilke, _Fundamentals of Data Visualization_
...
* Nicola Rennie, The Art of Data Visualization with ggplot2
...
* Package names are printed in **bold** and colored `r colorize("brown")`, for example
  `r pkg("ggplot2")`, `r pkg("lattice")` and the `r package("matlib")`. These uses generate citations like
  `r pkg("ggplot2", cite = TRUE)`. They are automatically indexed,
...
* A monospaced `typewriter` font is used in the text to refer to variable and function names, 
like `education` and `plot()`; also for R statement elements and keywords as well.
