...
This book is about graphical methods developed recently for multivariate data, and their uses in understanding relationships when there are _several_ aspects to be considered together. 

Data visualization methods for statistical analysis are well-developed 
and widely available in R for simple linear models with a _single_ outcome variable, as well as for more complex models with nonlinear effects, hierarchical data with observations grouped within larger units and so forth.
...
<!-- The smartdiagram source for this figure lives in latex/diagrams/fig-book-parts.tex,
     which builds both images/fig-book-parts.pdf (vector, CMYK — print) and
     images/fig-book-parts.png (600 dpi sRGB — HTML). Rebuild instructions in that file. -->
...
knitr::include_graphics(
  if (knitr::is_latex_output()) "images/fig-book-parts.pdf"
  else                          "images/fig-book-parts.png"
)
```
...
An [R Code appendix](https://friendly.github.io/Vis-MLM-book/Rcode.html) in the online version lists the R scripts used for most analyses and figures in the book.
:::
...
S programming language [@Becker-etal:88] and designed as a _functional_ language [@Backus1978]
...
R has learned a many lessons over time. Perhaps the most fundamental is "pipe" (`|>`) syntax, that allows you
to say `x |> log() |> diff() |> exp()` to express the same computation in an equivalent, but more readable way.

Then, R acquired the expressive language of the _Grammar of Graphics_ [@Wilkinson:99] to compose plots
from: aesthetics, mapping data variables to visual features, geoms, and using coherent themes to
set graphic styles, rather than a plethora of graphic `par()` settings.

However, in Wilkinson's formulation of the GoG, there was a black box labeled "Algebra", meant to
express the transformation of an imported dataset into variables that could be used directly in the graphics grammar.
This gap was later filled-in in R, first with Hadley Wickham's [@Wickham2011] "split-apply-combine strategy", where you break up a big problem into manageable pieces, operate on each piece independently and then put all the pieces back together, and later with
with the idea of making datasets "tidy" [@Wickham:2014:tidy], to make them easier to manipulate, model and visualize
...
between the idea of a graph and the visual result can still be narrowed. Happily, the design of `r pkg("ggplot2")` in
a "ggverse" of add-on and extension packages.[^ggplot-ext] continues to extend these ideas to wider usefulness.
...
data visualization ideas and techniques reflected here. Agnieska Kopinska, Gigi Luk, Touraj Amiri and Justeena Zaki-Azat
were TAs and RAs who contributed to my teaching and research. Most recently, Udi Alter, Michael Shiuming Truong and Gavin Klorfine worked as research assistants
and helped me in numerous ways with work on this book. In particular, Gavin's work on editing, solving technical problems and preparing the book for submission was invaluable.

Writing this book using [Quarto](https://quarto.org/) within the RStudio (now [Posit](https://posit.co/)) development environment
presented many technical challenges I had not encountered in previous books [@FriendlyMeyer:2016:DDAR].[^making-vis-mlm]
I am grateful to Mickaël Canouil,
Christophe Dervieux, Felix Benning and others in the [Quarto-Dev Discussion](https://github.com/orgs/quarto-dev/discussions) community who graciously helped me solve many issues,
and again to Michael Shiuming Truong and Gavin Klorfine who helped with this effort with incisive comments, suggestions and an eagle-eye to
typographic and programming details. In the final preparation stages, [Claude AI](https://claude.ai/)
was invaluable in many technical tasks like finding and fixing ill-formed citations and conversion of
RGB figures to CMYK for printing.

[^making-vis-mlm]: I recount some of these challenges in a blog post, [The Making of Vis-MLM](https://friendly.github.io/blog/posts/2026-04-making-vis-mlm/)

<!-- Should these use cite=TRUE? -->
The book also relies heavily on the graphic ideas and software of many R developers,
including:
Cory Brunson (`r pkg("ggeda")`);
Vincent Arel-Bundock (`r pkg("tinytable")`, `r pkg("marginaleffects")`)
Di Cook (`r pkg("tourr")`),
John Fox (`r pkg("car")`, `r pkg("effects")`, ...),
Daniel Lüdecke (`r pkg("easystats")`, `r pkg("ggeffects")`), and
Duncan Murdoch (`r pkg("rgl")`) whose work made this all easier, and
who replied to issues and feature requests on their packages.

Yihui Xie, with his packages `r pkg("knitr")`, `r pkg("xfun")`, and `r pkg("tinytex")` deserves thankful mention
because he provided the infrastructure
to write a book with R analysis and graphics integrated seamlessly into the text.
