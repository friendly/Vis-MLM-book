...
You can see this difference in the model formulas used in `r func("lm()")` compared with `r func("MASS::lda()")`, which fits a linear discriminant analysis (LDA) model: The outcomes on the right-hand side are `cbind(y1, y2, y3)` for the MANOVA but is simply `group` for a discriminant analysis.
...
[^candisc]: The function `r func("candisc::candisc()")` carries out a _generalized_ discriminant analysis, for one term in a multivariate
linear model. `r func("candisc::candiscList()")` does this for all terms in a model.
...
the groups are most widely separated, relative to the variability within each group.
...
These methods differ in that in PCA, the "juice" is the total variance of the cloud of points in $p$-dimensional
...
implemented in `r func("MASS::qda()")`.[^other-discrim]
...
package `r pkg("mda")`
as `r func("mda()")` and `r func("fda()")`, which have the same syntax and similar
methods to `r func("lda()")`.
...
problems like principal components [@Pearson:1901] and multivariate distance [@Mahalanobis1936] in geometric terms, with matrix algebra as a muse to express these ideas and handmaid to find solutions.
...
iris species. The method quickly found application in crainometry [@Fisher1936b], sex
classification of Egyptian mandible bones [@Martin1936] ...
...
<!-- fig.code: R/penguin/peng-lda-pred-new.R -->
...
By default, `r func("MASS::lda()")` uses the proportions of the three species as the prior probabilities, which are given in the printed output.
...
Fitting the LDA model to the `iris` dataset is quite simple, because the variables for petal and sepal
lengths and widths can be specified as `.` in the model formula, `Species ~ .`
...
And, the results are simpler also. From the singular values, extracted as `iris.lda$svd`, we see that
the first dimension, `LD1` accounts for 99.1% of the differences among the species means on these four variables.
```{r iris-svd}
svd <- iris.lda$svd
100 * round(svd^2/sum(svd^2), 3) |> 
  setNames(c("LD1", "LD2"))
```


<!-- fig.code: R/iris/iris-lda-ggplot.R -->
...
their predicted `class`, obtained from the `r func("MASS::predict.lda()")` method for an `"lda"`
object:
```{r class-table}
class_table <- table(
  actual    = peng$species,
...
A more sensitive assessment of accuracy is leave-one-out **cross validation**,
obtained using `CV = TRUE` in `MASS::lda()`. With this computer-intensive method, the function systematically holds out one observation, fits the model on the remaining $n$ cases and predicts the class of the excluded observation. The overall error rate can be calculated as:

```{r}
error_rate <- mean(peng.lda$class != peng$species)
```


\ix{cross validatation}
...
* `r func("predict_discrim()")`: a wrapper to `r func("MASS::predict.lda()")` returning a data.frame
* `r func("cor_lda()")` calculates _structure correlations_ between variables and discriminant scores
* `r func("scores.lda()")` a `r func("scores()")` method to extract discriminant scores

<!-- **TODO**: Need to re-write much of what is below to use these methods [DONE or IGNORED] -->
...
for the new observations. You can use `r func("candisc::predict_discrim()")`, which joins the measures in the
...
Using this result you can simply plot the original Penguin data for two variables and use `r func("geom_text()")` or
`r func("geom_label()")` to identify the observations of the
...
for `flipper_length` vs. `body_mass`. The key thing is that the call to `r func("geom_label()")` uses the predicted `species` and
coordinates for the new penguins from the `pred` dataset obtained from `r func("predict_discrim()")`.
...
My goal here is to illustrate plotting in discriminant space and show how the basic plot from `r func("plot.lda()")`
can be made more informative. First, `r func("predict_discrim()")` can also return the discriminant scores (`LD1`, `LD2`) for the new observations
...
Fortunately, `plot.lda()` allows a `panel =` argument, to control how the observations are represented.
...
discriminant scores, `LD1` and `LD2`, obtained using `scores=TRUE` in the call to `r func("predict_discrim()")`.
Data ellipses could then be added using `r func("car::dataEllipse()")` as follows (this plot is not shown).
...
predicted class of an observation at this point. In `ggplot2` this is easier, using `r func("geom_tile()")`.
...
in base R or `r func("geom_contour()")` in `ggplot2`.
...
The function `r func("klar::partimat()")` produces such partition plots for `r func("lda()")`, `r func("qda()")` as well as a number of other classification methods, including recursive partioning trees (`r func("rpart::rpart()")`), naive Bayes (`r func("e1071::naiveBayes()")`) and other methods. The graphs produced are
...
[^partimat-encoding]: In fairness, `r func("klaR::partimat()")` was designed to emphasize the classification accuracy shown in each pairwise plot,
...
giving an error rate of 0.204 in this plot. Two other panels also show high error rates.
...
The function `r func("marginaleffects::datagrid()")` makes this easy, automatically marginalizing the other variables.
...
Then, in the plotting steps, `r func("geom_tile()")` is used to display the predictions in the `pred_grid` dataset, using the penguin colors for the species. Points and other layers use the full `peng` dataset. I also calculate the 
...
#| fig-width: 10
#| out-width: "100%"
#| fig-cap: "**Linear discriminant** prediction regions for the three penguin species. Left: `bill_length` vs. `bill_depth`; right: `flipper_length` vs. `body mass`."
...
Plots showing decision regions such as @fig-peng-regions are made even easier using `r func("candisc::plot_discrim()")`.
...
<!-- fig.code: R/penguin/peng-lda-ggplot3.R -->
...
#| fig-width: 6
#| out-width: "60%"
#| fig-cap: "`plot_discrim()` plot of `bill_depth ~ bill_length` for the Penguin data showing the linear discriminant prediction regions for the three species."
means <- peng |>
  group_by(species) |>
...
<!-- fig.code: R/penguin/peng-lda-ggplot2.R -->
...
Then, I use `r func("gggda::geom_vector()")` to draw the variable vectors. Only the relative lengths of these matter,
...
which is done using `r func("coord_equal()")`.
...
As I noted at the outset, LDA is in some sense a twin of MANOVA, but with its emphasis flipped from 
...
In the printed table, the eigenvalues are the $\lambda_i$ of $\mathbf{H}\mathbf{E}^{-1}$ and their
percent contributions are the same as what we saw from the discriminant analysis given by `lda()`.
The significance tests printed are a variety called **step-down tests** [@Roy1957], which test the
...
In the code, `ellipse = TRUE` adds 68% data ellipses for the discriminant scores for each species,
...
When the covariance matrices $\mathbf{S}_k$ differ across the groups, the discriminant function @eq-lda-fun becomes
[@Rao1948]
...
QDA gets its name because the resulting decision boundaries are quadratic (curved) rather than straight lines.

Happily, with these developments, all the methods for analysis and visualization work just the same, if
you substitute `r func("MASS::qda()")` for `r func("MASS::lda()")` in the material above.

::: {#exm-penguin-qda}
**Quadratic penguins**

In @sec-eqcov, we saw that the Penguin groups had unequal covariance matrices when tested by
Box's $\mathcal{M}$ test, but these did not differ dramatically in visual displays (e.g., @fig-peng-covEllipse-pairs).
...
Plotting the quadratic prediction regions in data space in the same manner as in @fig-peng-regions, gives @fig-peng-qda-regions.

```{r}
#| label: fig-peng-qda-regions
#| echo: false
#| fig-height: 5
#| fig-width: 10
#| out-width: "100%"
#| fig-cap: "**Quadratic discriminant** prediction regions for the three penguin species. Left: `bill_length` vs. `bill_depth`; right: `flipper_length` vs. `body mass`."
means <- peng |>
  group_by(species) |>
...
You can see in @fig-peng-qda-regions that the decision boundaries for bill length and bill depth are only slightly curved. The penguin species are widely separated in this view and the extra complexity of the quadratic model doesn't make much difference in this view. 

In the right panel for flipper length and body mass,
QDA uses its' additional covariance parameters to construct more highly curved regions to try to
isolate the smaller differences between the Adélie and Chinstraps. The difference between these two species
appears more clearly in the low-rank canonical discriminant space of @fig-peng-candisc.

Was the QDA analysis worth the extra complexity? A simple way to find out is to re-run the LDA and QDA
analyses using cross validation (`CV = TRUE`) to assess the classification accuracy of each
...
So, for our penguin friends, we only gain an additional 0.3\% accuracy by using the quadratic method.
That's good, because LDA is easier for them to explain to new penguins, like Betsy and Dave, who come to an island and request membership in a particular species group. The Penguin Refugee Board could then
measure them and assign them to the most appropriate LDA group.
:::


<!-- **TODO**: Mention recursive partition methods as another topic, not covered here. -->
...
```{r child = "summary/Ch15-summary.qmd"}
```
