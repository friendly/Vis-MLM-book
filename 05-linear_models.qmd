...
Instead, we need some way to code those variables numerically. A key insight is that your choice of
a coding scheme changes the meaning of the model parameters, and allows you to perform different comparisons (test different statistical hypotheses) about the means of the category levels.
...
In R, categorical variables are called **factors** and a factor with $k$ levels is reflected in an
overall test with $k-1$ degrees of freedom.  This can be represented as $k-1$ comparisons among the 
factor level means.

Base R provides several coding schemes through the `contrasts()` function:

- `contr.treatment()`: Compares each level to a reference level using $k-1$ dummy variables. This is the default, and the reference level is taken as the first (in alphabetic or numerical order). You can change the reference level using `relevel()` or `reorder()` for the factor, or simply using `factor(A, levels = ...)`.

- `contr.sum()`: Compares each level to the grand mean. 
- `contr.helmert()`: Compares each level to the mean of _previous_ levels, treated as ordered categories
- `contr.poly()`: For ordered factors with _numeric_ levels, this creates orthogonal polynomial contrasts, representing the linear, quadratic, cubic ... trends in the factor means, as if these appear as $x, x^2, x^3, ...$ terms in a model with $x$ as a numeric variable.
