...
@tbl-techniques summarizes a variety of methods for linear models, classified by number of predictors and number of response variables, and whether these are quantitative vs. discrete. For the present purposes, the key columns are the first two, for the case
...
#| echo: false
#| out-width: "100%"
#| tbl-cap: "Techniques for linear models classified by number of predictors and number of response variables, and whether these are quantitative vs. discrete"
...
Not shown in @tbl-techniques are rows for cases where predictor variables include 
...
This expresses a response variable, $\mathbf{y} = (y_1, y_2, \dots , y_n)^\textsf{T}$ for $n$ observations, as a sum of terms involving $p$ _regressors_, $\mathbf{x}_1, \mathbf{x}_2, \dots , \mathbf{x}_p$, each of length $n$.
...
and the interaction term `x1:x2` is calculated as $x_1 \times x$2.
In algebraic notation (omitting the error term) this give

\begin{eqnarry*}
y & = & \beta_0 + \beta_1 x_1 +  \beta_2 x_2 +  \beta_1 x_1 *  \beta_2 x_2 \\
  & = & \beta_0 + (\beta_1 + \beta2 x_2) x_1 + \beta_2 x_2 \comma
}
\end{eqnarray*}

which means that the coefficient for $x_1$ in the model is not constant for all values of
$x_2$, but rather changes with the value of $x_2$.
...
their interaction, also identical to `A * B`.
...
* What about the same formula with factors `A, B, C, D`?[^ans]

[^ans]: (a) `y ~ .^2 -> y ~ x1 + x2 + x3 + x4 + x1:x2 + x1:x3 + x1:x4` 
