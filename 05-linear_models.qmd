...
I take up some of the details of these coding schemes below.
...
$x$, and raising it to successive powers, 0, 1, 2, 3.
```{r}
M <- outer(1:8, 0:3, `^`)
...
We can make the columns of `M` orthogonal using the Gram-Schmidt method,
where each successive column after the first is made orthogonal to all previous columns by subtracting their projections on the current column. Plotting these, as in @fig-poly shows that the coefficients
for a linear term plot as a line, those for $x^2$ follow a quadratic, and so forth.

```{r}
#| label: fig-poly
#| out-width: "70%"
#| fig-cap: "The coefficients for polynomial contrasts for linear (1), quadratic (2) and cubic (3) terms for a numeric variable $X$. The intercept or constant term is represented as 0."
