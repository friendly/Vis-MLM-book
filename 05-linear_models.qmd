...
a coding scheme changes the meaning of the model parameters, and allows you to perform different comparisons (test different statistical hypotheses) about the means of the category levels according to meaningful questions in your research
design.
...
The design matrix $\mathbf{X} = [\mathbf{1}, \mathbf{C}] = [\mathbf{1}, \mathbf{x}_b, \mathbf{x}_c, \mathbf{x}_c]$ includes the constant column $\mathbf{1}$ representing the intercept, averaging over the factor levels when there are other
terms in the model.
...
#### Other factor codings {.unnumbered}
