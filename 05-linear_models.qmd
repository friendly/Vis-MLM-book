...
Discrete explanatory variables, such as race, type of treatment or level of education
...
In R, categorical variables are called **factors** and the type of coding for a factor `A` can be retrieved via 
`contrasts(A)` or set by assigning a matrix specifying the comparisons of interest, as in `contrasts(A) <- C`.
