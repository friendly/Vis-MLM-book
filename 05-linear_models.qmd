...
\mathbf{y} & = \beta_0 + \beta_1 \mathbf{x}_1 + \beta_2 \mathbf{x}_2 + \cdots + \beta_p \mathbf{x}_p + \mathbf{\epsilon} \\
           & = \left[ \mathbf{1},\; \mathbf{x}_1,\; \mathbf{x}_2,\; \dots ,\; \mathbf{x}_p \right] \mathbf{\beta} + \mathbf{\epsilon} \\
           & = \mathbf{X}_{n \times (p+1)}\; \mathbf{\beta}_{(p+1) \times 1} + \mathbf{\epsilon}
\end{align*}
$$

The matrix $\mathbf{X}$ is called the _model matrix_ and contains the numerical representations of the 
_predictor_ variables.
...
* Interactions between predictors are represented as the direct products of the columns of 
