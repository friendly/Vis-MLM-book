...
This chapter is about seeing data in a different projection---a low-dimensional (usually 2D) space---that squeezes out the most juice
...
#| fig-align: center
#| out-width: "80%"
#| fig-cap: "A multivariate juicer takes data from possibly high-dimensional data space and transforms it to a lower-dimenional space in which important effects can be more easily seen. _Source_: Author image, using publicly available clipart."
...
2D space of principal components and _simultaneously_ show how the original variables
...
I'm using the term _multivariate juicer_ here to refer to the wider class of **dimension reduction** techniques, used for various purposes in data analysis and visualization. PCA is the simplest example and illustrates the general ideas.
...
\ixon{principal components analysis}
...
\ix{principal components analysis!springs}
...
sum of squared lengths of the connecting springs, and also minimizes the kinetic energy in the system.
...
\ix{principal components analysis!geometry}
...
PCA axes (`r func("heplots::ellipse.axes()")`). @fig-workers-pca also shows the 
...
In R, PCA is most easily carried out using `r func("stats::prcomp()")` or `r func("stats::princomp()")`
or similar functions in other packages such as `r func("FactomineR::PCA()")`.
...
The object, `crime.pca` returned by `prcomp()` is a list of the following elements:
...
`r func("broom::tidy()")` provides methods to unify extraction of these values.
...
`r func("stats::plot()")` would give a bar plot of the variances of the components, however `r func("ggbiplot::ggscreeplot()")` gives nicer and more flexible displays as shown in @fig-crime-ggscreeplot.
...
For plotting, it is more convenient to use `r func("broom::augment()")` which
...
an aspect ratio of 1.0 (via `r func("coord_fixed()")`)
...
Then, I plot these using `r func("geom_segment()")`, taking some care to use arrows
from the origin with a nice shape and add `r func("geom_text()")` labels for the variables positioned slightly to the right. Again, `r func("coord_fixed()")` ensures equal scales for the axes, which is important because we want to interpret the angles between the variable vectors and the 
PCA coordinate axes.
...
the correlation of that variable with the $j$th principal component. Thus,
    ...
    * A variable uncorrelated with a component is perpendicular to it.
...
that is shown in this space. This means that:
    ...
    * Variable vectors at right angles are approximately uncorrelated, while those pointing in opposite directions are negatively correlated; $r = -1$ if they are at 180$^o$.
...
\ixoff{principal components analysis}
...
::: {.callout-note title="Biplot scaling"}
The choice of $\alpha$ apportions the singular values between the row scores
($\mathbf{A} = \mathbf{U}\boldsymbol{\Lambda}^\alpha$) and the column loadings
($\mathbf{B} = \mathbf{V}\boldsymbol{\Lambda}^{1-\alpha}$), giving three useful interpretable cases
for how the relations among the row points and variable vectors can be understood:

- **$\alpha = 1$** (_row-principal_ / GH biplot): rows in _principal coordinates_, columns in
  _standard coordinates_. Distances between row points approximate Mahalanobis distances among
  observations; angles between column vectors approximate correlations among variables [@Gabriel:71].
- **$\alpha = 0$** (_column-principal_ / JK biplot): columns in principal coordinates, rows in
  standard coordinates. Useful when the primary interest is relationships among the variables
  viewed as points [@Gabriel:71].
- **$\alpha = 1/2$** (_symmetric biplot_): singular values split equally between rows and columns.
  Neither set has a clean distance interpretation, but the display balances both simultaneously
  [@Greenacre:2010:biplots].
  
For general, exploratory overviews where you want a balanced, visual summary of both the observations and the variables without prioritizing one over the other, the symmetric version is a useful default, which I follow here.
:::
...
There are a large number of R packages providing biplots. The most basic, `r func("stats::biplot()")`, provides methods for `"prcomp"` and `"princomp"` objects. Among other packages, `r pkg("factoextra", cite=TRUE)`, an extension of `r pkg("FactoMineR", cite=TRUE)`,
is perhaps the most comprehensive and provides `ggplot2` graphics.
In addition to biplot methods for
quantitative data using PCA (`r func("fviz_pca()")`), it offers biplots for categorical data using correspondence analysis
(`r func("fviz_ca()")`) and multiple correspondence analysis (`r func("fviz_mca()")`); factor analysis with mixed quantitative and categorical variables (`r func("fviz_famd()")`) and cluster analysis (`r func("fviz_cluster()")`).
...
`r func("factoextra::get_pca_var()")` calculates a number of tables from a `"prcomp"` or similar object.
...
using `r func("factoextra::fviz_contrib()")`. The dashed horizontal lines are at the average value for each dimension.
...
#| out-width: "80%"
#| fig-cap: "Contributions of the crime variables to dimensions 1 (left) & 2 (right) of the PCA solution. The dashed horizontal lines show the average contribution for each dimension."
...
in a style similar to @fig-fviz-contrib using `r func("factoextra::fviz_cos2()")`.
...
relations among a set of variables, you can use _other available data_ to help interpret what
...
for analysis using `r func("factoMineR::PCA()")`.
```{r crime-joined}
crime_joined <-
...
The `PCA()` result can then be plotted using `r func("FactoMiner::plot()")`
or various `factoextra` functions like `r func("fviz_pca_var()")` for a plot of the variable vectors or
`r func("fviz_pca_biplot()")` for a biplot. When a `quanti.sup` component is present, supplementary variables are
also shown in the displays.

For simplicity I use `r func("FactoMiner::plot()")` here and only show the variable vectors. For consistency with earlier
...
Recall that from earlier analyses, I interpreted the dominant PC1 dimension as reflecting overall
...
shown in @fig-ReavenMiller-3d. For this, I use `r func("car::scatter3d()")` which
...
`r func("car::scatter3d()")` uses the `r package("rgl", cite=TRUE)` to render 3D graphics on a display device,
which means that it has facilities for perspective, lighting and other visual properties. You can interactively zoom
in or out or rotate the display in any of the three dimensions and use `r func("rgl::spin3d()")`
to animate rotations around any axes and record this as a `movie3d()`.
...
<!-- fig.code: R/diabetes/diabetes-3d.R -->
...
Then, find the centroids of the component scores and use `r func("geom_label()")` to plot the group labels.
```{r}
...
What can we see here, and how does it relate to the artist's depiction shown earlier in @fig-ReavenMiller-3d?
...
in which relationships among variables are at least approximately linear, and 
...
(`r func("dist()")`). The basic function is `r func("MASS::isoMDS()")`.[^vegan]
...
provides `r func("vegan::metaMDS()")` which allows a wide range of distance measures ...
...
To plot the 2D solution, I'll use `r func("ggpubr::ggscatter()")` here because it handles grouping, provides concentration ellipses 
...
\ixoff{multidimensional scaling}
...
`r func("Rtsne::Rtsne()")` finds the locations of the points in the low-D space, of dimension `k=2` by default.
...
#| out-width: "80%"
#| fig-cap: "t-SNE representation of the Diabetes data with data ellipses for the diagnostic groups."
...
The `state_length` argument `r func("transition_states()")` controls the relative 
...
_Motor Trend_ and consists of fuel consumption (`mpg`) and 10 aspects of automobile
...
and are outlined in the plot using `r func("corrplot::corrRect()")`. What became clear in the corrplot is that `qsec`, the time to drive a quarter-mile from a dead start didn't quite fit this pattern, so I highlighted it separately.
...
When PCA (or the SVD) is used on facial images, you can think of the process as generating **eigenfaces** [@Turk1991],
...
small dimensions as illustrated (with slightly different SVD notation) in @fig-image-compression-SVD.

```{r}
#| label: fig-image-compression-SVD
#| out-width: "90%"
#| fig-cap: "Image compression represented in terms of the SVD. _Source_: Tomio Kobayashi, Eigenvalues vs Singular Values, [https://bit.ly/42B27Qy](https://bit.ly/42B27Qy)"
knitr::include_graphics("images/image-compression-SVD.png")
```
...
#| echo: false
#| out-width: "35%"
#| fig-cap: "640 x 954 black and white image of the _Mona Lisa_. _Source_: [Wikipedia](https://bit.ly/3Rgo41f)"
...
An image can be imported using `r func("imager::load.image()")` which creates a `"cimg"` object, 
...
We can do this using `r func("tidyr::pivot_wider()")`, giving a result with 640 rows and 954 columns.
```{r mona-reshape-wide}
...
largest eigenvalues. The function `r func("approx_pca()")` does this, and also undoes the scaling
...
using ggplot. In the code below, the `r func("approx_pca()")` function is run for each of the 9 values specified by `n_pcs`
...
In `ggplot()`, each  is plotted using `r func("geom_raster()")`, using `value` to as the fill color.
A quirk of images imported to R is that origin is taken as the upper left corner, so the
Y axis scale needs to be reversed. The 9 images are then plotted together using `r func("facet_wrap()")`.
...
In this figure, with 4-5 components most people would recognize this as a blurry image of the world's most famous portrait. It is certainly clear that this is the Mona Lisa with 10--15 components.
...
::: {.content-visible when-format="pdf"}
The online edition of this book shows this more clearly in an [animated graphic](https://friendly.github.io/Vis-MLM-book/04-pca-biplot.html#fig-outlier-animation) rotating the points smoothly from data space to PCA space. It uses `r func("heplots::interpPlot()")` to interpolate linearly from the positions of the points
in data space to their locations in PCA space.
:::

::: {.content-visible when-format="html"}
To see this more clearly, @fig-outlier-animation shows an animation of the rotation from data space to PCA space. This uses `r func("heplots::interpPlot()")` to interpolate linearly from the positions of the points
in data space to their locations in PCA space.
...
#| out-width: "80%"
#| echo: false
#| fig-cap: "Animation of rotation from data space to PCA space." 
