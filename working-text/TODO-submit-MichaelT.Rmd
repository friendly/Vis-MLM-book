...
@Michael: Because you're using markdown here, you could also use ~~strikethrough~~ corrected text
in your comments.
...
[I'm considering splitting this and hived-off the stuff from parallel coords on into a new chapter, presently `04-higher.qmd` which would become
Chapter 5, wit all the following chapters renamed.]
...
[Yes, they are more generally useful]

- Perhaps change period to colon?  [**Where is this in the text?**]
...
The text for the left plot Figure 8.8 is clipped [FIXED]

> Because there are multiple
degree of freedom terms in the model, `car::vif()` calculates GVIFs here.

- I think this deserves greater explanation as to what 'degrees of freedom' means here and why GVIF applies in this case, but not the others? [FIXED]
...
- Is it really necessary to call $w$ the linear combination and $a$ the weights? It seems counter-intuitive, perhaps $w$ should be the weights instead? Shouldn't it also be fine to change $w$ to $t$ or $T$?

- It's implied, but also not clear, whether the $a$ used later on i.r.t the Eigenvector and more is the same $a$ that was used to denote weights
...
- The rendering here seems to be broken for html. Perhaps new line and delete tab?

> to give an **exact** $F$ test with the correct
sampling distribution:

- Worth describing what an exact test is and why it matters [added footnote]
...
- Is there a way to get effect sizes here to quantify the differences between groups? And the effects within groups? 

- Part of me wonders if it'd be better to put chapter 9 after either Chapters 10 or 11. I feel that some of the content in chapter 9 would require knowing Ch 10/11 to really understand it, but it's not super important.

- It does not seem like 'canonical space' is defined in this chapter, yet it seems like a very important concept to understand the last bits of the final sections of this chapter. 

## Chapter 10: Multivariate Linear Models

> # Multivariate Linear Models {#sec-mlm-review}

- The name of the section header is off [??? Not sure what this refers to]

> A weaker form, called **exogeneity** merely asserts that the predictors are independent of (uncorrelated with) the errors, $\Epsilon$.

- What's an example of a violation of exogeneity? Of when a predictor is correlated with the errors in $Y$?

> Implicit in the above is perhaps the most important assumption---that the model has been _correctly specified_. This means:

- I think it's worth arguing that even if the model isn't correctly specified, there is still value in the multivariate linear model. The problem is that we rarely a theory so strong that would warrant the use of something like SEM -- but it's not because your theory is weak that you should constrain yourself to a univariate multiple linear regressi
...
- I think it would be better to explicitly state how the univariate case can be extended into the multivariate case. Perhaps even showing how the univariate case is a special case of the multivariate formula SSP formula would be nice.


> $\left(\widehat {\mathbf{Y}}^{\top}\widehat{\mathbf{Y}} - n\overline{\mathbf{y}}\,\overline{\mathbf{y}}^{\top} \right)$ 

- I guess this is $\mathbf{SSP}_{H}$? If so, this needs to be explicitly stated, right as the substition is made. Right now the substitution seems to be implied and seems to show up later in equation 10.6.

> in the off-diagonal, where the sign is important: In $\text{SSP}_H$ the the negative covariance

- in the off-diagonal, where the sign is important: In $\text{SSP}_H$ the ~~the~~ negative covariance

> These are the $p$ values $\lambda$ which solve the determinant equation

- There is something about the grammar of this sentence which seems off

- Why is it that figure 10.2 uses the Basic Math and Word Problems, but Section 10.2 is about Dog food? The flip-flopping is confusing me...

- Can we have a graph for what the meaning of the final result of section 10.2.3 is? Specifically this one 
...
> degrees of freedom, there are $s > 1$

- A refresher on what $s$ means would be useful to know why there are $s > 1$ possible dimensions. 

> Nested dicontomy contrasts are always _orthogonal_, and therefore 
correspond to statistically independent tests. We are effectively taking a three

- 'dichotomy' instead of dicontomy?
- For pedagogy, what's an example of a non-orthogonal test? What's an example of the way that the multivariate test statistic can be abused incorrectly using non-orthogonal non-independent tests?
...
- If we're using short form, then perhaps `educ`, `antisoc` and `hyperact` should all be quoted in back-ticks to give that special font? [FIXED]

> #### Fiting models {.unnumbered}
...
- Howevever, in a one-way MANOVA design with more than three response variables, it is difficult

- What a canonical space itself is does not seem to be defined..?
- Are canonical scores the same as $\mathbf{Z}$?

## Chapter 12: Visualizing Equality of Covariance Matrices

> Just as the Welsh $t$-test [@Welch:1947]

- Welch $t$-test [FIXED]

> putting numbers to the visual conslusion from

- visual conclusion

> ## Multivariate analog of Levine's test {#sec-multivar-levene}

- The spelling of Levene's test is sometimes swapped for Levine throughout [FIXED]

## Chapter 13: Multivariate Influence and Robust Estimation

- Misterious or Mysterious?

- Surely, we could get a higher quality plot than the hand-drawn plot of Figure 13.9 somehow? Section 13.5 also does not seem to be completed.
