...
-   Quarto formatting is a bit off in web-format, it's showing
    `::: {.cell layout-align=“center”}` **Fixed; need a blank line b4
    code**
-   Someway to disable ligatures on R-pipe (\|\>)? I think this may
    confuse those unfamiliar with fonts **Use `monofont: "Fira mono"`,
    not `monofont: "Fira code"`**
...
appropriate with inappropriate interpretations would be nice
...
meaningof the larger graph. - Latex right before section 3.2.2 is
incomplete -- not rendering correctly:
$\(\sqrt{\mathbf{v}_1}, \sqrt{\mathbf{v}_1\), …$
...
of `coef(lm(prestige ~ income, data=Prestige))[2]`.
...
-   Perhaps making Figure 4.4 go into a continuous loop of a GIF would
    ...
    -   It's also a bit ambiguous what the 'horizontal and vertical'
        dimensions are in a moving 3D graph
...
-   Could be nice to clarify whether the data need to already be
    centered/scaled or whether the function can handle it.
...
-   It would be nice to have an explanation for why
    ...
    correlated and not have equal variance.
...
'Southeast of the plot' - 'Murder' is also cutoff
