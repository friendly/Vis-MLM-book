...
Researchers who have read standard treatments of linear models
...
#| echo: false
#| out-width: "100%"
#| fig-cap: "A scene from one of the Where's Waldo books. Waldo wears a red-striped shirt, but far too many of the other figures in the scene have horizontal red stripes, making it very difficult to find him among all the distractors. This is often the problem with collinearity diagnostics. _Source_: Retrieved from https://bit.ly/48KPcOo"
...
You can see that the coefficients for Year and GNP are shrunk considerably. Differences from the $\beta$ value at $k =0$ represent the bias needed to achieve more stable estimates.
...
But a bigger problem is that these univariate plots are the **wrong kind** of plot! They show the trends in increased bias associated with larger $k$, but they do not show the accompanying increase in _precision_ (decrease in variance) achieved by allowing a bit of bias.
...
As can be seen, the coefficients for each pair of predictors trace a path generally in toward the origin (0,0), and the covariance ellipses get smaller, indicating increased precision. Sometimes, these paths are rather direct, but it takes a peculiar route in the case of population and GNP.
...
the covariance matrix $\mathcal{V}_k \equiv \widehat{\Var} (\widehat{\boldsymbol{\beta}}^{\mathrm{RR}}_k)$:
...
* `det` $ = \log{| \mathcal{V}_k |}$ is an overall measure of variance of the coefficients. It measures the linearized volume of the covariance ellipsoid and corresponds conceptually to Wilks' Lambda criterion. 
...
Plotting shrinkage against variance each other gives a direct view of the tradeoff.
