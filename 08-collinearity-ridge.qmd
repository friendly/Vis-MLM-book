...
[@HoerlKennard:1970a].
More recently they developed to a larger class of model selection methods, of which the
...
### Properties of rigde regression
...
\text{RSS}(k) = (\mathbf{y}-\mathbf{X} \mathbf{\beta}) \trans  (\mathbf{y}-\mathbf{X} \boldsymbol{\beta}) + k \boldsymbol{\beta}\trans\boldsymbol{\beta} \quad\quad (k \ge 0)
 \comma 
$$ {#eq-ridgeRSS}
where the penalty restrict the coefficients to some squared length $\boldsymbol{\beta}\trans \boldsymbol{\beta} \le t(k)$.

The geometry of ridge regession is illustrated in @fig-ridge-demo for two coefficients
$\boldsymbol{\beta} = (\beta_1, \beta_2)$. The `r colorize("circles", "blue")`
...
The `r colorize("ellipses", "red")` show contours of the covariance ellipse of
$\widehat{\boldsymbol{\beta}}^{\mathrm{OLS}}$.
...
circles or ellipses first kiss as they expand [@Friendly-etal:ellipses:2013].
...
By analogy with OLS, where the hat matrix, $\mat{H} = \mat{X} (\mat{X}\trans \mat{X})^{-1} \mat{X}\trans$
reflects degrees of freedom $\textrm{df} = \trace{H} = p$
corresponding to the $p$ parameters, the effective degrees of freedom for ridge regression \citep{Hastie-etal:elem:2001} is

\begin{align*}
 \textrm{df}_k 
    = & \textrm{tr}[\mat{X} (\mat{X}\trans \mat{X} + k \mat{I})^{-1} \mat{X}\trans] \\
    = & \sum_i^p \textrm{df}_k(i) = \sum_i^p \left( \frac{d_i^2}{d_i^2 + k} \right) \period
\end{align} {#eq-dfk}

$ \textrm{df}_k$ is a monotone decreasing function of $k$, and hence any set of ridge constants
can be specified in terms of equivalent $\textrm{df}_k$.
