...
#| echo: false
#| out-width: "100%"
#| fig-cap: "A scene from one of the Where's Waldo books. Waldo wears a red-striped shirt, but far too many of the other figures in the scene have horizontal red stripes, making it very difficult to find him among all the distractors. This is often the problem with collinearity diagnostics."
...
```
The estimated coefficients  can be extracted using `coef()` applied to each model:
...
(`car::dataEllipse()`) in the corresponding fitted model. In @fig-collin-data-beta, I specify the x, y limits for each plot
...
variance of $x_j$ that is not explainable from the others.
