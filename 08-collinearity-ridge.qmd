...
can be specified in terms of equivalent $\text{df}_k$.
...
You can see that the coefficients for Year and GNP are shrunk considerably, but differences from the value at $k =0$ represent the bias needed to achieve more stable estimates.
...
The problem is that these univariate plots are the **wrong kind** of plot! They show the trends in increased bias associated with larger $k$, but they do not show the accompanying increase in _precision_ (decrease in variance). For that, we need to consider the variances and covariances of the estimated coefficients. The univariate trace plot is the wrong graphic form for what is essentially a multivariate problem, where we would like to visualize how both coefficients and their variances change with $k$.
