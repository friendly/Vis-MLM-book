...
In @ridge-low-rank we see that, once again, PCA-related techniques like the biplot can be insightful---here, to 
...
because it is multiples that matter here. It uses intervals of 1-5, 5-10, 10+ to highlight low, medium and
high variance inflation with colored bands.

```{r}
#| label: fig-cars-check-collin
#| out-width: "80%"
#| fig-cap: "Variance inflation plot"
...
though the underlying message is the same. Number of cylinders, engine displacement and weight are the 
bad boys. They help to pin Waldo down a bit, but to find him, we need a few more diagnostics and better
graphical methods.
:::
...
A classic example for ridge regression is Longley’s (1967) data, consisting of 7 economic variables, observed yearly from 1947 to 1962 (n=16), in the dataset `r dataset("longley")`. 
...
Shrinkage values can be specified using $k$ (where $k = 0$ corresponds to OLS) or the equivalent degrees of freedom $ \text{df}_k$ (@eq-dfk). (The function uses the notation $\lambda \equiv k$, so the argument is `lambda`.)
...
But a bigger problem is that these univariate plots are the **wrong kind** of plot! They show the trends in increased bias (toward smaller $\mid \beta \mid$) associated with larger $k$, but they do not show the accompanying increase in _precision_ (decrease in variance) achieved by allowing a bit of bias.
...
and their size and shape indicate sampling variance, $\widehat{\text{Var}} (\mathbf{\widehat{\beta}}_{ij})$. Here, I plot those for GNP against four of the other predictors. The `plot()` method for `"ridge"` objects plots these ellipses for a pair of variables.
...
You can see them shrink in one direction in the last two columns and rows.
...
#| fig-height: 10
#| out-width: "100%"
#| fig-cap: "All pairwise bivariate ridge plots shown in PCA space."
pairs(plridge)
```
...
the case if the predictors were uncorrelated. Collinearity can be seen
