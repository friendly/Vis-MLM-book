...
\widehat{\Var} (\widehat{\boldsymbol{\beta}}^{\mathrm{OLS}}) & = & 
    \widehat{\sigma}^2 (\mathbf{X}\trans \mathbf{X})^{-1}.
\end{array*} {#eq-OLS-beta-var}
...
\end{eqnarray}
where $\mat{G}_k = \left[\mat{I} + k (\mat{X}\trans \mat{X})^{-1} \right] ^{-1}$, the $(p \times p)$ _shrinkage_ matrix.
