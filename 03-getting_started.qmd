...
> _Getting information from a table is like extracting sunlight from a cucumber._ ---@FarquharFarquhar:91

At the time the Farquhar brothers wrote this pithy aphorism, graphical methods for understanding data had advanced considerably, but most statisticians still relied on tables to communicate their results, prompting the Farquhar complaint. Most data tables
we see today are designed for looking something up, like the number of measles cases in Ontario compared to other
provinces. Tables are not usually designed to reveal _patterns, trends or anomalies_, although this can
be accomplished with modern table generating software such as the `r pkg("tinytable")` or `r pkg("gt")` packages.[^tables]
...
can be used to show unusual values as in "heatmaps".
...
In the last half of the 19th Century, the idea of correlation was
developed [@Galton:1886; @Pearson:1896] and the period, roughly 1860--1890, dubbed
the "Golden Age of Graphics" [@Funkhouser:1937; @Friendly:2008:golden]
...
to illustrate the importance of plotting the graphs before analyzing and model building, and the effect of unusual observations on fitted models.
...
@fig-anscombe1 is an enhanced version of Anscombe's plot of these data, adding
...
knitr::include_graphics("images/anscombe1.png")
```

This figure is produced as follows, using a single call to `r func("ggplot()")`, faceted by `dataset`.
As we will see later (@sec-data-ellipse), the data ellipse 
(produced by `r func("stat_ellipse()")`)
<!-- (produced by `stat_ellipse()`) -->
reflects the correlation between the variables.
\ix{data ellipse}
<!-- \ix{stat_ellipse()@\texttt{stat\_ellipse()}} -->
...
The method Anscombe used to compose his quartet is unknown, but it turns out that there is
...
As shown in 
`r ifelse(knitr::is_html_output(), '@fig-datasaurus-html', '@fig-datasaurus-pdf')`
the sets include a star, cross, circle, bullseye,
...
```{r}
#| label: fig-datasaurus-html
#| out-width: "90%"
#| echo: false
#| fig-cap: "Animation of the Dinosaur Dozen datasets. _Source_: [https://youtu.be/It4UA75z_KQ](https://youtu.be/It4UA75z_KQ)"
...
```{r}
#| label: fig-datasaurus-pdf
#| out-width: "100%"
#| echo: false
#| fig-cap: "Plots of the Dinosaur Dozen datasets. _Source_: [Selçuk Korkmaz on X](https://x.com/selcukorkmaz/status/1864583253253927156)"
...
The `r package("quartets", cite=TRUE)` contains these and other variations on this theme.
\ix{quartet!causal}
\ix{quartet!Rashamon}
...
In `r pkg("ggplot2")` this can easily be done by reversing the `x` and `y` aesthetics.
...
In @fig-davis-reg2, this discrepant observation again stands out like a sore thumb, but
this makes very little difference in the fitted line for females.
...
but also help identify their _reasons_ and suggest corrective _action_.
What went wrong here? Examination of the original data sheets showed that this woman (case 12) mistakenly switched
the values, recording her reported weight in the box for measured weight and vice versa. 
The puzzling numerical results were crystal clear in the graphs.
...
> _Although we often hear that data speak for themselves, their voices can be soft and sly._---Frederick Mosteller (1983), _Beginning Statistics with Data Analysis_, p. 234.
...
#| out-width: "80%"
#| echo: false
#| fig-cap: "Congressman Alexander Pirnie (R-NY) drawing the first capsule for the Selective Service draft, Dec 1, 1969. _Source_: Wikipedia [https://bit.ly/45c23sB](https://bit.ly/45c23sB)"
...
In an attempt to make the selection process also transparent, the proceedings were
...
ggplot(Draft1970, aes(x = Day, y = Rank)) +
  ...
  geom_smooth(method = "lm", formula = y~1,
              se = FALSE, col = "black", 
              fill="grey", linetype = "dashed", alpha=0.6) +
...
changes in lottery rank over the year. Using a visual cue like a broken axis is one way to avoid misinterpretation.
For `ggplot2` graphs, the `r package("ggbreak")` is useful for this purpose.
...
put in cardboard boxes, one for each month and these were carefully emptied into the glass container in order of month: Jan., Feb., through Dec., and gently shaken in atop the pile already there.  
...
Of course, if they had R, the entire process could have been done using `r func("sample()")`:
...
My lottery rank would have been 274![^personal]
...
* __dimension reduction plots__: plot representations of the data in a space of fewer dimensions than the number of variables in the analysis (@sec-pca-biplot).
Simple examples include principal components analysis (PCA) and the related biplots (@sec-biplot)
and multidimensional scaling (MDS) methods. This is the topic of @sec-pca-biplot, but this powerful idea runs through the rest of the book. For example, multivariate linear models can be projected into a 2D _canonical space_ (@sec-candisc) that shows the effects for many response variables (@sec-candisc).
I refer to such plots as __multivariate juicers__, because they can squeeze the essence of your data into low-D package,
enhancing the flavor.
\ix{multivariate juicer}
...
* __conditional plots__: Other model plots try to take into account that 
a fitted model may involve more variables than can be shown in a static 2D plot.
Some examples of these are _added variable plots_ (@sec-avplots), and conditional _effect plots_ (@sec-effect-displays), both of which attempt to show the net relation of two focal variables, _controlling_ or adjusting for other variables in a model by holding them fixed.
\ix{added-variable plots}
\ix{effect plots}

* __diagnostic plots__: indicating potential problems with the fitted model. For linear models, these include residual plots, influence plots, plots for testing homogeneity of variance and so forth, illustrated in @sec-diagnostic-plots.
Plots for diagnosing problems with multivariate models are discussed in @sec-model-diagnostics-MLM. One simple example is described below.
\ix{diagnostic plots}
...
For example, the plot of the Davis data in @fig-davis-reg1
effectively fits a separate regression line for males and females, which is expressed by the model formula
`repwt ~ weight * sex`. Having fit the model using `r func("lm()")`, the plot method for a `"lm"` object produces a
...
#| fig-height: 8
#| out-width: "90%"
#| fig-cap: "Regression quartet: Four diagnostic plots for the linear model fit to the Davis data. The details of these plots are discussed in @sec-regression-quartet."
