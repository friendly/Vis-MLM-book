...
effects are _interaction_ terms, as in `y ~ group + x + group:x`, where the term `g:x` fits a different slope for each group and the grouping variable is often called a _moderator_ variable. 
...
```{r child="child/data-ellipse.qmd"}
```
