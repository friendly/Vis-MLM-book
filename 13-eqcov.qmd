...
\ixon{covariance matrices!equality}
\ixon{covariance matrices!heterogeneity}
...
\ix{discriminant analysis!quadratic}
...
means of the auxiliary variable $z_{ij} = | y_{ij} - \bar{y}_i |$.
\ixon{Levene's test}
...
These deviations from a group central value can be calculated using `r func("heplots::colDevs()")` and the central value can be
a function, like `mean`, `median` or an anonymous one like `function(x) mean(x, trim = 0.1))` that trims 10%
off each side of the distribution. With a response `Y` Levene-type tests can then be performed "by hand" as follows:
...
The function `r func("car::leveneTest()")` does this, so we could examine whether the variances are equal in the Penguin variables, one at a time,
like so:
```{r levene-peng1}
...
To gain some insight into the problem of homogeneity of variance it is helpful to see how the situation looks in terms of data.
For the Penguin data, it might be simplest just to look at boxplots of the variables and try to see whether the 
**widths** of the
...
\ixon{peng@\texttt{peng} data}
\ixon{datasets!peng@\texttt{peng}}

To preview a main example, @fig-peng-covEllipse0 shows data ellipses for the
main size variables in the Penguins data (`peng`).
<!-- \ix{datasets!peng} -->
<!-- \ix(peng data) -->
`r func("heplots::covEllipses()")` is specialized for viewing the relations among 
...
\ixoff{peng@\texttt{peng} data}
\ixoff{datasets!peng@\texttt{peng}}
...
Multivariate thinking suggests that we calculate some measure of "size" of each $\mathbf{S}_i$,
...
In practice, there are various transformations of the value of $\mathcal{M}$ to yield a
...
This section applies this to visualizing and testing the heterogeneity of covariance matrices in multivariate designs.
...
covariance from the other two species, and that while _Virginica_ and _Versicolor_ both have similar
...
source(here::here("R/util/text_usr.R"))    # text in user coords
...
text_usr(0.05, 0.95, "(a) Uncentered", pos = 4, cex = 1.5)
...
covEllipses(iris.pca$x, iris$Species,
  ...
  cex.lab = 1.5,
  label.pos = c(1, 3, 3, 0), asp=1)
text_usr(0.95, 0.95, "(b) Centered", pos = 2, cex = 1.5)
```
...
Here we see that the correlation for _Virginica_ is of opposite sign from the
other two groups.
...
\ix{generalized variance}
\ix{precision matrix}
\ix{eigenvalue!maximum}
...
\ixon{Levene's test!multivariate}
\ix{MANOVA}
...
the Levene-type MANOVA in 1D or 2D. 
...
\ixoff{Levene's test!multivariate}
\ixoff{covariance matrices!equality}
\ixoff{covariance matrices!heterogeneity}

```{r child = "summary/Ch13-summary.qmd"}
```
