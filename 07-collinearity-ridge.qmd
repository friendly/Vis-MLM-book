...
$$
with sampling variances and covariances $\text{Var} (\widehat{\mathbf{b}}) = \sigma^2 \times (\mathbf{X}^\textsf{T} \mathbf{X})^{-1}$ and $\sigma^2$ is the variance of the residuals $\mathbf{\epsilon}$, estimated by the
...
the solution depends on the determinant $|\,\mathbf{X}^\textsf{T} \mathbf{X}\,|$, which approaches 0
