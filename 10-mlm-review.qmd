...
$$F = \frac{SS_H/df_h}{SS_E/df_e} = \frac{\mathsf{Var}(H)}{\mathsf{Var}(E)}
$$
assesses "how big" $SS_H$ is, relative to $SS_E$, the variance accounted for by a hypothesized model or model terms relative to
error variance.
...
$$
However, when the hypothesized model terms have $\text{df}_h$ degrees of freedom, $\H$ is of rank $\text{df}_h$,
so only $s=\min(p, df_h)$ eigenvalues can be non-zero.
...
possible dimensions in which $\H$ can be large relative to $\E$.
