...
The structure of the model matrix $\mathbf{X}$ is the same as the univariate linear model, and may therefore contain, 
...
each term in the model is returned as a named list `SSP` and the error $\mathbf{SSP}_{E}$ is
...
\begin{align*}
L_1 & = (\mu_O + \mu_N) - (\mu_M + \mu_A) & \rightarrow\: & \mathbf{c}_1 = 
    \begin{pmatrix} 
     1 &  1 & -1 & -1 
    \end{pmatrix} \\
...
within each of these sets. Such contrasts are always _orthogonal_, and therefore 
