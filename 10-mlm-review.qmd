...
Non Follow Through (NFT), but here I simply focus on the relations between outcome scores
...
The dataset is given in `heplots::schooldata`.
...
Rather than a complete $8 \times 8$ scatterplot matrix, it is useful here to examine the scatterplots just for each $y$ variable against each of the
predictors in $\mathbf{X}$.[^ggduo] 
...
Similarly `etasq()` for an MLM gives an $R^2$-like measure of the partial association accounted for each
of the predictor terms in the model. In spite of the overwhelming significance of the first three
predictors, their variance accounted for is more modest, and highest for parent's occupation.

```{r school-etasq}
etasq(school.mod)
```

Finally, given that we found a few noteworthy points in @fig-schooldata-cqplot, a $\chi^2$ QQ plot
...
is not unusual in the fitted model. It is a high-leverage point, but appeared to be well-fitted in all the
simple regression, except in those for `teacher`.
