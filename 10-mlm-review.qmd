...
to adjust differences among the groups.
...
a mixture of factor variables and quantitative predictors, called "covariates" in this setting.
In this wider context, there are two flavors of analysis with different emphasis on the
factors or the covariates.

* true ANCOVA/MANOVA: Attention is centered on the differences between the group means.

* homogeneity of regression: Here the focus is on the regression relations between the $\mathbf{y}$s
and the predictors, but we might also want to determine if the regression slopes are the same for
...
### Example: Paired-associate tasks and academic performance
...
by William D. Rohwer at the University of California, Berkeley.
...
There were three outcome measures: A student achievement test (SAT),
and two visually-based measures, the Peabody Picture Vocabulary test (PPVT) and the Raven
Progressive Matrices test (Raven). Four paired-associate tasks were used, which differed in the
syntactic and semantic relationship between the stimulus and response terms in each pair. 
These are `n` (named), `s` (sentence), `ns` (named sentence), `na` (named action) and
`ss` (sentence still).

His data, originally from @Timm:75, p. 281,313, is given in `heplots::Rohwer`. But there's a MANCOVA wrinkle: Performance on the academic tasks is well-known to vary with socioeconomic status of the parents or the school they attend. A simple design was to collect data from children in two
schools, one in a low SES neighborhood and the other a high SES one.
