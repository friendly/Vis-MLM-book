...
The _minimum covariance determinant (MCD)_ estimator  is a robust estimator with high breakdown value and bounded influence. It looks for the subset of size $h$, whose covariance matrix has the smallest determinant, where $\lceil n/2 \rceil < h < n$ controls the robustness. The `distancePlot()` function
