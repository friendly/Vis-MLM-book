...
Yet the combination of these ideas remains elusive, ... _mention special cases_ ... And graphical methods ???
...
In this context, just as in univariate analysis there are variously named techniques (ANOVA, regression) that can be applied to several outcomes, depending on the structure of the predictors at hand. For instance, with one or more continuous predictors and multiple response variables, you could use **multivariate** multiple regression (MMRA) to obtain estimates useful for prediction. 
...
The main goal of this chapter is to describe the details of the extension of ...

Before considering the details and examples that apply to MANOVA and MMRA, 
it is useful to consider the features of the multivariate linear model of which these cases are examples.
...
(A slight hiccup in notation is that the _uppercase_ for the Greek Beta ($\boldsymbol{\beta}$)
