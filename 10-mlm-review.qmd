...
This can be understood as a simple extension of the univariate linear model, with the main difference being that there are multiple response variables considered **together**, instead of just one, analysed alone.
...
The main goal of this chapter is to describe the details of the extension of univariate linear models to the case
of multiple outcome measures.
Before considering the details and examples that apply to MANOVA and MMRA, 
it is useful to consider the general features of the multivariate linear model of which these cases are examples.

**TODO**: Offer defense against @Huang2019 and others here; cite @Huberty-Morris1989
...
where an overall test for group differences is applied before testing individual comparisons to avoid problems of

* More importantly, fitting separate univariate models does not take _correlations_ among the $\mathbf{y}$s into account. It might be the case that the response variables are all essentially measuring the same thing, but perhaps weakly. If so, the multivariate approach can pool strength across the outcomes to detect their common relations to the predictors, giving greater power.
...
However, as in univariate models, the MLM is relatively robust 
...
* **Completeness**: No relevant predictors have been omitted from the model. For example in the coffee, stress example (@sec-coffee-stress), omitting stress from the model biases the effect of coffee on heart disease.
...
### Testing contrasts and linear hypotheses {#sec-contrasts}
...
univariate multiple regression to the situation where there are $p$ response variables.
...
When there are more than a few predictors, approaches like hierarchical regression and
stepwise selection can be used to eliminate uninformative predictors for each response,
This gives a different interpretation of the models for each response, based on the predictors included.
...
**Overall tests**
In a one-way ANOVA, to comtrol for multiple testing, it is common practice to carry out an overall $F$-test to see if the group means differ collectively before testing comparison between specific groups.
Similarly, in univariate multiple regression, researchers sometimes report an overall $F$-test or test of the
$R^2$ so they can reject the nypothesis that _all_ predictors have no effect, before considering them individually.
...
Youth (NLSY) is a series of annual surveys conducted by the 
U.S. Department of Labor to examine the transition of young people into the labor force. 
@Curran1997 
-->

The dataset `heplots::NLSY` comes from a small part of the National Longitudinal Survey of
...
#### Exploratory plots {.unnumbered}
To begin, I would examine some scatterplots and univariate displays. I'll start with density plots for all the variables to see the shapes of their distributions. From @fig-NLSY-density we see that math and reading scores
are positively skewed, anti-social and hyperactivity have distributions highly concentrated in the lower scores.
As we would suspect, father's income is extremely positively skewed, and crys out for a log transformation.
Father's education is reasonably symmetric, but highly peaked at 12 years of schooling in this sample.
...
The `Anova()` results above give multivariate tests of the contributions of each predictor separately
to explaining reading and math and how they vary together. To get an overall test of the global
null hypothesis $\mathcal{H}_0 : \mathbf{B} = \mathbf{0}$ for _all_ predictors, you can use
`linearHypothesis()`:
...
#| fig-height: 6
#| out-width: "70%"
#| fig-cap: "Bivariate coefficient plot for reading and math with 95% confidence ellipses"
...
In @fig-NLSY-coefplot1, the confidence ellipses for `log2(income)` and `educ` both exclude the
...
we would conclude that for reading log(income) is not a significant predictor. 
...
to the previous $y$ variables, and on the right-hand side to the $x$s in the previous model.
...
The dataset, given in `heplots::schooldata` contains observations for `r nrow(heplots::schooldata)` schools.[^schoolsites]

[^schoolsites]: In this, schools 1--49 were PFT sites and the remaining sites 50--70 were NFT.
A separate dataset `heplots::schoolsites` provides other information on the schools, such as
...
we can facet using `facet_grid(yvar ~ xvar)`.
...
@sec-eqcov. Here, I consider the assumed multivariate normality of residuals and multivariate influence.
...
**TODO**: Results from MVN package are too unweildly here. Replace with something different.
...
From this, you can see that cases 44 and 59 are actually quite troublesome here, but for different reasons.
...
Consequently, one appropriate action would be to re-fit the model, ignoring these cases, in what
I call a **sensitivity test**: Do the main conclusions change with ...
