...
But **wait**: did you check the assumptions of the MLM? ...
...
#### Multivariate influence {.unnumbered}

Again, what we see in simple scatterplots can be misleading because they ignore all the other
variables in a model. The multivariate extension of the diagnostic measures of leverage and influence,
and influence plots (@sec-leverage) is provided by the `r package("mvinfluence", cite = TRUE)`.
@fig-schoolmod-infl shows one form of an influence plot for the `school.mod` model.
...
#| fig-height: 6
#| out-width: "70%"
#| fig-cap: "Influence plot for the `schooldat` multivariate regression model."
