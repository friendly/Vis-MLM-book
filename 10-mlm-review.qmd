...
for all responses with each of the predictor PA tasks. This is similar in spirit to what we would get if we fit a separate  multivariate regression model for each of the groups, but parameterized differently:
it gives separate slope estimates for each of the groups rather than estimates of the difference
in slopes.

This approach is *not* generally recommended for testing hypotheses because it
lacks power (smaller sample sizes in each group than in a combined analysis)
and does not allow hypotheses about equality of slopes and intercepts
to be tested directly. Here, I simply illustrate the mechanics of fitting separate models
