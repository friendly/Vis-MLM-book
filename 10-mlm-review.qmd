...
* **Error-free $\mathbf{X}$**: The predictors, $\mathbf{X}$, are fixed and measured without error.  The discussion in @sec-meas-error illustrates how measurement error biases (toward 0) the estimated effects of predictors. A weaker form, called **exogeneity** merely asserts that the predictors are independent of (uncorrelated with) the errors, $\Epsilon$. [^errors-in-variables]

[^errors-in-variables]: Among the extensions of the classical OLS framework, _errors in variables_ models ...
