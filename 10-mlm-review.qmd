...
This can be assessed visually using a $\chi^2$ QQ plot
...
$H_0: \mathbf{B} = 0$ (excluding the row for the intercept). 
...
In the univariate case, the overall $F$-test of $H_0: \boldsymbol{\beta} = \mathbf{0}$
...
The dataset `Parenting` comprises 60 fathers selected from three `group`s of $n = 20$ each: (a) fathers of a child with _no disabilities_; (b) fathers with a _physically_ disabled child; (c) fathers with a _mentally_ disabled child. 
...
More specific questions are: (a) Do the fathers of normal children differ from the other two groups
on average? (b) Do the physical and mental groups differ? These can be tested using contrasts,
...
#| out-width: "90%"
#| fig-cap: Boxplots of scores on the three parenting scales 
...
ggplot(parenting_long, aes(x=group, y=value, fill=group)) +
  ...
  stat_summary(fun=mean, color="white", geom="point", size=2) +
...
If these were all the same, the data ellipses should have roughly the same size and orientation,
...
Let's proceed to fit the multivariate model predicting all three scales from `group`:
...
`car::Anova()` gives the overall test of the `group` effect, using Pillai's trace for the test...

```{r}
Anova(parenting.mod)
```

The `summary()` method ... gives more details, including the $\mathbf{H}$ and $\mathbf{E}$ matrices
and all four test statistics.
