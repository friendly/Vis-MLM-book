...
contributions over and above the baseline model. You can do this using `update()`:
...
Non Follow Through (NFT), but here I focus on the relations between outcome scores
on tests of reading, mathematics and selfesteem in relation to
...
be sufficiently revealing. I tried a number of different methods and found a couple that were
interesting and useful.

Multivariate normality is **not** required for all the variables in $\mathbf{Y}$ and $\mathbf{X}$,
but for MMRA problems, sometimes an initial $\chi^2$ QQ plot provides a handy way to flag 
possibly unusual values as the analysis proceeds. ...
...
# get the case ID numberfs
outliers <- rownames(res) |> as.numeric()
```

In this case, it is useful to examine the scatterplots for each $y$ variable against each of the
predictors in $\mathbf{X}$. To prepare for this with `ggplot2`, it is necessary to 
reshape the data to long format twice, once for the $x$ variables and again for the $y$ responses.
That way, we can facet by `yvar ~ xvar`.
...
we can facet these by `yvar ~ xvar`.
...
#| fig-height: 6
#| out-width: "100%"
#| fig-cap: "Scatterplots of each of the three response variables against each of the five predictors in the `schooldata` dataset. "
