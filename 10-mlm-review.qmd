...
$\mathbf{B} = ( \mathbf{b}_0^\top , \mathbf{b}_1^\top , \dots \mathbf{b}_j^\top \dots, \mathbf{b}_p )^\top$, or more explicitly
...
$$
L = \mathbf{c}^\top \boldsymbol{\mu} = \sum_i c_i \mu_i \quad\text{such that}\quad \Sigma c_i = 0
$$
...
if $\mathbf{C}^\top \mathbf{C}$ is a diagonal matrix.
...
The joint test of the linear coefficients $\boldsymbol{\beta}_1 = (\beta_{1,\text{anx}},  \beta_{1,\text{dep}})^\top$ for anxiety and depression, 
