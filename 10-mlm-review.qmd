...
so only $s=\min(p, \text{df}_h)$ eigenvalues can be non-zero.
...
The same idea can be applied to test the difference between any pair of _nested_ models---the added contribution of terms in a larger model relative to a smaller model containing a subet of terms.
...
\end{center}
```
Test statistics for multivariate tests
:::
...
interpreted in terms of two uncorrelated dimensions, perhaps labeled overall competancy and emphasis
