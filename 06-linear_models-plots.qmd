...
is a generally useful solution to this problem. These plots are nearly always easier to understand than
...
`predictorEffects()` (for one or more). For a model `mod` with formula `y ~ x1 + x2 + x3 + x1:x2`
`predictorEffects(mod, ~ x1)` recognizes that an interaction is present and
calculates the fitted values for combinations of `x1` and `x2`. This returns an object of
...
Each plot shows the fitted regression relation and a pointwise confidence band using the
...
#| fig-width: 8
#| out-width: "100%"
#| fig-cap: Predictor effect plot for all terms in the model
allEffects(prestige.mod3) |>
  plot()
```

The effect for women, holding education, income and type constant looks to be quite strong.
...
The interaction is graphs showing separate curves for the three levels of type.
...
to `predictorEffects()`. As with C+R plots, a smooth loess curve gives a visual assessment of
...
understand the interaction by plotting income on a log scale.  ...
