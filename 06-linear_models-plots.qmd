...
As we've seen (**TODO**: Coffee data) simple, _marginal_ scatterplots of a response $y$ against _each_ of several predictors $x_1, x_2, \dots$ can be misleading because each one ignores the other predictors.
...
The _added-variable_ plot ("AV plot", also called _partial regression_ plot, @LarsenMcCleary:72; @Cook:93) is the multivariate analog of a
...
The construction of an AV plot is conceptually very simple. For variable $x_i$, imagine that we fit
two supplementary regressions:

* Regress $y$ on $\mathbf{X_{(-i)}}$, the model matrix of all of the regressors except $x_i$. 
By definition, the residuals from this regression,  $r_y(x_i) = y \,\vert\, \text{others}$,
are the part of $y$ that cannot be
...
These residuals, $r_x(x_i) = x_i \,\vert\, \text{others}$, give the part of $x_i$ that cannot be explained by the others, and so are uncorrelated with them.

The AV plot is then just a simple scatterplot of these residuals, $y \,\vert\, \text{others}$
on the vertical axis, and $x_i \,\vert\, \text{others}$ on the horizontal.
In practice, it is unnecessary to run the auxilliary regressions this way.
...
Note that `y` here contains both the current predictor, $x_i$ and the response $y$, so the
...
* Consequentially, the standard deviation of the (vertical) residuals in the AV plot is the same as $s = \sqrt(MSE)$ in the full model and the standard error of a coefficient is $\text{SE}(b_i) = s / \sqrt{\Sigma r_x(x_i)^2}$.

* The horizontal positions, $x_i \,\vert\, \text{others}$, of points adjust for all other predictors, and so we can see points at the extreme left and right as unusual in relation to the others. If these points are also badly fitted (large residuals), we can see their influence on the fitted relation in the full model. AV plots thus provide visual displays of leverage and influence on each of the regression coefficients.
