...
observations, where each point of  $(\mathbf{x}_i^\star, \mathbf{y}^\star)$ is the image of $(\mathbf{x}_i, \vec{y})$ along the path of the marginal regression. The variability in the response and in the focal
predictor are both reduced, leaving only the uncontaminated relation of $\mathbf{y}$ with $\mathbf{x}_i$.

These plots are similar in spirit to the ARES plot ("_A_dding _RE_gressors _S_moothly") proposed
...
the fit of a larger model. They used linear interpolation controlled by a slider,
...
is to fit a smoothed curve through the points in the AV plot.
