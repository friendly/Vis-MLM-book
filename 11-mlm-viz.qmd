...
The model contemplated earlier examined how the three response variables, resistance to `tear`, film `gloss` and the `opacity` of the film varied with the two experimental factors, `rate` of extrusion and amount of some `additive`, both at two levels, labeled `High` and `Low`. 
