...
relations in terms of linear combinations of each. That is, the goal is to find  weights
...
Geometrically, these linear combinations are vectors representing projections in the observation space of the $x$ and $y$ variables, and canonical correlation analysis can also be thought of as
minimizing the angle between these vectors or maximizing the cosine of this angle. This is illustrated in @fig-cancor-diagram (with a slight difference in notation relative to the discussion below.)

```{r}
#| label: fig-cancor-diagram
#| echo: false
#| out-width: "100%"
#| fig-cap: "Diagram illustrating canonical correlation. For two $y$ variables, all linear combinations are vectors in their plane, and similarly for the $y$ variables. Maximizing the correlation between linear combinations of each is equivalent to making the angle $\\theta$ between them as small as possible, or maximizing $\\cos({\\theta})$, shown in the diagram at the right. The thick grey arrow indignates that the two planes should be overlaid at a common origin. _Source_: From a Cross-Validated discussion by user 'ttnphns', https://bit.ly/4dgq2cp"
knitr::include_graphics("images/cancor-diagram.jpg")
```



Specifically, we want to find one set of weights $\mathbf{a}_1$ for the $x$ variables and another
for the $y$ variables to give the linear combinations $\mathbf{u}_1$ and $\mathbf{u}_1$,
...
\end{eqnarray*}
such that the correlation $\rho_1 = \textrm{corr}(\mathbf{u}_1, \mathbf{v}_1)$ is maximized. 
...
what they common is a space of $s = \min(p, q)$ dimensions. 
...
account for 98.6%, so you might think that that is
...
in the correlations between the pairs of canonical scores, ...

The `plot()` method plots the canonical X scores against the Y scores for a given dimension
(selected by the `which` argument):
...
#| fig-height: 8
#| out-width: "70%"
#| fig-cap: "Plots of canonical scores on the first canonical dimension for the `schooldata`, including the influential cases."
...
The variable vectors shown in these plots are intended only to show the correlations of Y variables with the canonical dimensions. Only their _relative lengths_ and angles with respect to the Y canonical dimensions have meaning. Relative lengths correspond to proportions of variance accounted for in the Y canonical dimensions plotted; angles between the variable vectors and the canonical axes correspond to the structure correlations. The absolute lengths of these vectors are typically manipulated by the `scale` argument to provide better visual resolution and labeling for the variables.
