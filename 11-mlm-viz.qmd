...
#| echo: false
#| out-width: "100%"
#| fig-cap: "Diagram illustrating canonical correlation. For two $y$ variables, all linear combinations are vectors in their plane, and similarly for the $x$ variables. Maximizing the correlation between linear combinations of each is equivalent to making the angle $\\theta$ between them as small as possible, or maximizing $\\cos({\\theta})$, shown in the diagram at the right. The thick grey arrow indignates that the two planes should be overlaid at a common origin. _Source_: Re-drawn by Udi Alter following a Cross-Validated discussion by user 'ttnphns', https://bit.ly/4dgq2cp"
