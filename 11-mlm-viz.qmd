...
for one set of variables, say $\mathbf{X}$ not to predict each of the other set 
$\mathbf{Y} =[\mathbf{y}_1, \mathbf{y}_2, \dots]$ _individually_, but rather also finding weights
for these which is most highly correlated with the linear combination of the $\mathbf{x}$s.
...
chapter. There we fit the multivariate regression model predicting students scores on
...
related to reading than to mathematics. Visits to school and number of teachers are not significant in
this plot, but both are positively correlated with reading and math. The parent time counseling measure,
while also insignificant,
tilts in the opposite direction, having different signs for reading and math.

In the `pairs()` plot for all three responses (@fig-school-heplot2), we see something different in the relations for self-esteem. ...
