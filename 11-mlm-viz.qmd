...
#| fig-height: 6
#| out-width: "60%"
#| fig-cap: "HE plot for the `dogfood` data, showing the means of the four groups, which generates the $\\mathbf{H}$ ellipse for the effect of `formula`. The $\\mathbf{H}$ ellipse labeled 'Error' shows the within-group variances and covariance."
...
From the analysis in @sec-sums-of-squares, we found the $\mathbf{H}$ matrix in the `dogfood.mod` to be
...
The key to this is the eigenvalue decomposition, $\mathbf{H}\mathbf{E}^{-1} \lambda_i = \lambda_i \vec{v}_i$
(@eq-he-eigen)
