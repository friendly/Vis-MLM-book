...
but the other terms are included in the model to control for them.[^control]

[^control]: As in univariate linear models, any factors or covariates ignored in the model formula have their effects
...
separation between Average and Unattractive photos.The right panel shows 
...
As before, a canonical analysis compresses the information about a collection of responses into fewer dimensions.
...
[^candisc-terms]: For MANOVA designs there is a separate analysis for each term in the model. ...
...
#| fig-height: 7
#| out-width: "80%"
#| fig-cap: "Canonical discriminant plot for the factor `Attr` ..."
...
are most strongly associated with this. Ratings on `sociable` are related to both dimensions, but more so with
`Can2`.
