...
@Huang2019 and others have criticized these methods as (a) difficult to understand because they are framed in terms of linear combinations of the the responses;
...
These difficulties in understanding can, I believe, be cured by accessible
...
The HE plot framework described below provides powerful graphic methods available in easy used software.
...
[^heplots-vignettes]: See the heplots vignettes [HE plot MANOVA Examples](http://friendly.github.io/heplots/articles/HE_manova.html) and [HE plot MMRA Examples](http://friendly.github.io/heplots/articles/HE_mmra.html).
...
<!-- figure-code: R/dogfood-quartet.R -->

<!-- **TODO**: Re-run code producing this with better axis limits for (c) -->
...
$c = \sqrt{2 F_{2, n-2}^{0.68}}$ for 2D plots and $c = \sqrt{3 F_{3, n-3}^{0.68}}$ for 3D.
...
The key to this is the eigenvalue decomposition, $\mathbf{H}\mathbf{E}^{-1} \lambda_i = \lambda_i \mathbf{v}_i$
(@eq-he-eigen)
...
and $s=\min ( p, df_h )$ dimensions, the degrees of freedom for the hypothesis.
...
data and increased visual understanding of 
...
A significant advantage of HE plots is that the main effects and interactions
can be overlaid in the same plot showing how each term contributes to
assessment of differences among the groups. 

I illustrate this below for a simple
$2 \times 2$ factorial design with three response variables. 
...
In this view, the effect of extrusion `rate` is highly significant, with the high level giving larger tear resistance and lower gloss on average. High level of `additive` produces greater tear resistance and higher gloss.  The interaction
effect, `rate:additive`, while non-significant, points nearly entirely in the direction of `gloss`.

**Understanding interactions**

To understand an interaction effect, the simplest thing is to plot the cell means and error bars in a traditional line plot
with one factor on the horizontal axis and the other as separate lines within the panel. Here (@fig-plastic-ggline)
I use `ggpubr::ggline()`,
...
in a legend. Wrappers for `ggplot2` simplify some things but can't accommodate this kind of customization.
...
#| fig-height: 6
#| out-width: "100%"
#| fig-cap: Line plots of means and their standard errors for the response `tear` (left) and `gloss` (right)"
p1 <- ggline(Plastic, 
  x = "rate", y = "tear",
...
For resistance to `tear`, the lines are parallel, so there is no interaction, which is why the `rate:additive` effect had
no horizontal component in @fig-plastic-HE1. In the panel for `gloss`, the means for `additive` don't differ at high `rate`,
but do substantially at the low extrusion rate.

But what if you really wanted to show the means for the combinations of rate and additive in an HE plot?
...
-->
* `heplots::Hernior`: Recovery from Elective Herniorrhaphy -> HE_mmra vignette -- Use for exercise
...
individual predictors in @fig-NLSY-heplot2 is of worth noting here. Those for the
