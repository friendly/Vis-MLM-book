...
(similar to biplots for a PCA), as shown below (@fit-iris-candisc).
...
An industrial experiment was conducted to determine the optimum conditions for extruding plastic film. Of interest were three responses: resistance to `tear`, film `gloss` and the `opacity` of the film. Two factors were manipulated:
`rate` of extrusion and amount of some `additive`,
both of these being set to two values, `High` and `Low`. The dataset `heplots::Plastic`  comes from
@JohnsonWichern:92.
...
`heplot()`, to overlay the plots for significance and effect size scaling in
a single plot (using `add = TRUE`).
