...
<!--
::: {.callout-note}
This will become a numbered chapter to allow cross-references, but that entails renaming the chapter & figures files
:::
-->

# Warm-up Exercises {#sec-prelude}

The aim of this book is to blow your mind! OK, that's way too hyperbolic.
At the very least, I hope to expand your imagination and take visual thinking to higher dimensions.
...
(also known for modernizing Mendeleev's periodic table). They reflect the time in the early 1900s
...
[e.g., @Haskell:1919; @Peddle:1910] and college courses [@Costelloe:1915].
...
* Edward W. Maunder [-@Maunder:1904] plotted the occurrence of sunspots by their latitude over time. He observed a consistent, repeating pattern like the wings of a butterflies, with sunspots drifting toward the equator
...
Abbott published this under the pseudonym, "A Square", suggesting his place in the hierarchy.
...
line (a woman)?  How does a Triangle "see" a Hexagon or even an infinitely-sided Circle?
...
#| fig-align: center
#| out-width: "90%"
#| fig-cap: "A 2D Flatlander seeing a sphere as it passes through Flatland. The line, labeled 'My Eye' indicates what the Flatlander would see. _Source_: @Abbott:1884"
...
of seeing in 1, 2, 3 dimensions, and how the idea of motion over time (one more dimension) could 
...
For Abbott, the way for a citizen of any world to imagine one more dimension was to consider how a
...
[^1-sagan]: In his famous TV series, _Cosmos_, Carl Sagan provides [an intriguing video presentation](https://youtu.be/UnURElCzGc0) of Flatland and the 4th dimension.
...
<!-- Author-drawn TikZ figure; source in latex/diagrams/fig-1D-4D.tex, which
     builds both images/1D-4D.pdf (vector, CMYK — print) and images/1D-4D.png
     (600 dpi sRGB — HTML). Rebuild instructions in that file. -->
...
knitr::include_graphics(
  if (knitr::is_latex_output()) "images/1D-4D.pdf"
  else                          "images/1D-4D.png"
)
```
...
#| echo: false
#| out-width: "100%"
#| fig-cap: "Five frames from an animation of a tesseract, a cube changing through a 4th dimension over time _Source_: ediacura, [YouTube](https://www.youtube.com/watch?v=5xN4DxdiFrs)."
...
Our familiar three-dimensional space, of length, width, and height had formed the backbone of Euclidean geometry for millennia.
...
The first three variables are the lengths of geometric features of 3848 observed sampled pollen grains -- in the x, y, and z dimensions: a `ridge` along x, a `nub` in the y
...
Lest the EUREKA example seem contrived (which it admittedly is), truly _multivariate_ visualization has played an important role in quite a few scientific discoveries. Among these, Francis Galton's [-@Galton:1863] discovery of the
...
They found a peculiar ''horseshoe'' shape in this relation (shown in @fig-diabetes1), about which they could only speculate: perhaps individuals with the best glucose tolerance also had the lowest levels of insulin as a response to an oral dose of glucose; perhaps those with low glucose response could secrete higher levels of insulin; perhaps those who were low on both glucose and insulin responses followed some other mechanism.
...
measure of the efficiency of insulin use in the body, where large values mean
...
rarely developed the overt form. Our understanding of the etiology of Type 2 diabetes was
