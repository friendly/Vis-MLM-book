...
\ixon{multivariate linear model}

@sec-Hotelling introduced the essential ideas of multivariate models in the context of a two-group design
...
[^mglm]: There's a bit of a puzzle here---a gap in methods---and therefore an opportunity. The classical linear models fit by `lm()` extend naturally to non-gaussian data  via `glm()` which provides for other families
...
\ix{multivariate linear model!assumptions}
...
Let $\boldsymbol{\epsilon}_{i}^{\prime}$ represent the $i$th row of $\Epsilon$. Then it is assumed that:

* **Normality**: The residuals, $\boldsymbol{\epsilon}_{i}^{\prime}$ are distributed as multivariate normal, 
...
of Mahalanobis squared distance against their corresponding $\chi^2_p$ values for $p$ degrees of freedom using `r func("heplots::cqplot()")`.

* **Homoscedasticity**: The error-covariance matrix $\boldsymbol{\Sigma}$ is constant across all observations and grouping factors.
...
[^exogeneity]: A class of violation of exogeneity is **omitted variable bias**, which occurs
...
\ix{multivariate linear model!fitting}
...
In the presence of possible outliers, robust methods are available for univariate linear models (e.g., `r func("MASS::rlm()")`). So too, `r func("heplots::robmlm()")` provides robust
...
<!-- fig.code: R/dogfood/dogfood-ex.R -->
...
is carried out using `r func("car::Anova()")`.
```{r dogfood-aov}
...
\ix{multivariate linear model!sums of squares}
...
the result returned by `r func("car::Anova()")`, where the hypothesis $\mathbf{SSP}_{H}$ for
...
to the amount eaten. In $\text{SSP}_E$ the covariance is slightly positive. This might reflect
...
$\mathbf{z}_i = v_{i1} \mathbf{y}_1 + v_{i2} \mathbf{y}_2 + \cdots + v_{ip} \mathbf{y}_p$ which produces the largest
...
\ix{multivariate linear model!test statistics}
...
asymptotically equivalent. The default choice in `r func("car::Anova()")` of `test.statistic = "Pillai"` reflects
...
and are calculated using `r func("heplots::etasq()")`.
...
for all pairwise differences among groups (using Bonferroni or other corrections for multiplicity),
...
You can test such contrasts or any other hypotheses involving linear combinations of the coefficients  using `r func("car::linearHypothesis()")`. Here, `"formulac1"` refers to the contrast `c1` for the difference between Ours and Theirs. Note that because this is a 1 df test, all four test statistics
...
\end{pmatrix}}
$$
\ixoff{multivariate linear model}
...
A multivariate analysis would show a large difference among groups.
From a rough visual inspection, it seems that means differ on the math test $Y_1$, with the
physics group outperforming the other two. On the reading test $Y_2$, however, it might turn out that the three
...
of the responses. These can be extracted from an MLM using `r func("stats::summary.aov()")` and they give tests
...
`r func("heplots::glance()")` method for a multivariate model object. The code below selects just the $R^2$ and
...
`r func("car::Anova()")`, shown below, gives the overall multivariate test $\mathcal{H}_0: \mathbf{B} = 0$ of the `group` effect, that the groups don't differ on _any_ of the response variables. Note that
...
them above. The data structure returned contains nested elements which can be extracted more easily from the object using `r func("purrr::pluck()")`:
...
`"group1"`. The function `r func("car::linearHypothesis()")` carries out the multivariate test
...
It is also useful to examine within-group correlations using `covEllipses()`, as shown in
...
The bivariate means in @fig-addhealth-covellipse increase with grade, but the relationship is curved rather than strictly linear.
...
$$
\HO : \boldsymbol{\mu}_7 = \boldsymbol{\mu}_8 = \cdots = \boldsymbol{\mu}_{12} \; ,
$$
...
We fit the MANOVA model, and test the grade effect using `r func("car::Anova()")`.
...
of linear and nonlinear trends, `r func("car::linearHypothesis()")` on the coefficients
of model `AH.mlm`.
...
responses separately, `r func("heplots::uniStats()")` (or `r func("heplots::glance.mlm()")`) gives some answers, including $R^2$ and univariate $F$ tests.
...
#### Plotting main effects and interactions {.unnumbered}
...
error bars with `r func("geom_errorbar()")` you must calculate the standard errors and work out the upper and lower limits.
In @fig-plastic-ggline
I use `r func("ggpubr::ggline()")`,
...
comparisons in a plot, for example using `r func("ggpubr::geom_pwc()")` for pairwise comparisons, or to use 
...
\ixoff{MANOVA!factorial}
\ixoff{MANOVA}

## MRA $\rightarrow$ MMRA {#sec-MRA-to-MMRA}

\ixon{multiple regression!multivariate}

When all predictor variables are quantitative, the MLM @eq-mlm becomes the extension of
univariate multiple regression analysis (MRA) to the situation where there are $p$ response variables (MMRA).
...
dangerous in that they can increase false positives or drop variables that should, on logical grounds, be included in the model.
...
Similarly, the case of multivariate linear models, some consider it necessary to reject the multivariate
...
Youth](https://www.nlsinfo.org/content/cohorts/nlsy97), a series of annual surveys conducted by the 
...
In this analysis the `math` and `read` scores are taken as the outcome variables.[^other-vars]
...
#### Exploratory plots {.unnumbered}
To begin, I would examine some scatterplots and univariate displays. I'll start with density plots for all the variables to see the shapes of their distributions, with rug plots at the bottom to show where the observations are located.
From @fig-NLSY-density we see that math and reading scores
are positively skewed. Additionally, anti-social and hyperactivity have distributions highly concentrated in the lower scores.
...
The function `r func("heplots::noteworthy()")` provides a variety of methods to identify such noteworthy points
...
`r func("heplots::coefplot.mlm()")` gives displays of the coefficients for a given _pair_ of response variables.
...
#### Behavioral measures {.unnumbered}
...
\ixoff{multiple regression!multivariate}

## Model diagnostics for MLMs {#sec-model-diagnostics-MLM}

\ixon{diagnostic plots!multivariate}
...
Such plots are produced by `r func("heplots::distPlot()")`. Running this on the `school.mod` model
...
\ix{robust estimation}

### Multivariate influence {#sec-multivar-infl}

\ixon{influence!multivariate}

<!-- **TODO** Sort out coverage here vs. @sec-influence-robust DONE: With a forward reference -->
...
This topic is considered in greater detail in @sec-influence-robust.
...
<!-- {r child = "child/10-MANCOVA.qmd"} -->

\ixoff{influence!multivariate}
\ixoff{diagnostic plots!multivariate}
...
<!-- **TODO**: Consider moving this to @sec-vis-mlm and use much of the heplots MMRA vignette. IGNORED-->
...
* **true ANCOVA/MANOVA**: Attention is centered on the _differences among group means_, but controlling for any difference in the covariate(s). This requires assuming that the slopes
for the groups are all the same.

* **homogeneity of regression**: Here the focus is on the _regression relations_ between the $\mathbf{y}$s
...
<!-- **TODO**: Complete this, explaining how MANCOVA requires equal slopes for covariates, while homogeneity of regression tests that. ... DONE -->
...
Rohwer's data, taken  from @Timm:75, is given in `r dataset("Rohwer")`. But there's a MANCOVA wrinkle: Performance on the academic tasks is well-known to vary with socioeconomic status of the parents or the school they attend. A simple design was to collect data from children in two
...
or `r func("heplots::uniStats()")`. All are significantly related, but the `PPVT` measure has the largest $R^2$ by far.
...
<!-- **TODO**: For interpretation, it would be nice to know how many items were used for each of the PA tasks. The range of `na` goes up to 35, but others are less. IGNORED: It was never possible to find the actual tasks used for this dataset -->

#### Adjusted means {.unnumbered}

\ix{MANCOVA!adjusted means}
...
<!-- **TODO**: do this with a CI for the effects. NO: Make this an exercise -->

#### Homogeneity of regression {.unnumbered}

\ixon{MANCOVA!homogeneity of regression}
...
\ixoff{MANCOVA!homogeneity of regression}
