...
Here are some colors for testing these ideas and solutions. 
...
Define a function `colorize(text, color)` to print `text` in `color`. If `color` is missing, assume `text` is the color when I just
want to color the color name itself.

This uses `knitr::is_*` functions to decide whether to use HTML (`<span ...>`) or LaTeX (`\textcolor{...}`) coding to achieve this.
...
The text used in this document for this, showing how `colorize()` is used in inline text, is:
...
This approach doesn't work in figure captions, because the calls to `colorize()` must be evaluated as expression before they are valid for Quarto.
What I have used so far is `!expr paste()` to interpolate the colorize calls in `#| fig-cap:`

```{r}
#| echo: fenced
#| eval: false
#| label: fig-Salaries-lm
#| fig-cap: !expr paste("Scatterplot of Salary vs. years since PhD, showing", 
#|   colorize("linear", "red"), 
#|   "and", colorize("quadratic", "darkgreen"), 
...
For just using the color names themselves in figure captions, this is easier if I define variables for colors like this somewhere in my document:
...
Then, I can use them inside `glue::glue()` more simply as `{blue}`, `{green}`, ..., which interpolates their values into
the caption string. Using `paste()` with long captions often gave errors, where I had left out a `,` somewhere.
...
For legibility, it may work better to use the supplied color as background color, rather than text color, as RStudio
does.

Here's an analogous function, `colorize_bg()` to do this. In the HTML output, it uses `<span style='background-color:` rather than
...
All colors should now be legible! Well, this may need a tweak. In HTML, `r colorize2("Chinstrap", "darkgreen")` looks OK, but the text in
`r colorize2("green")` is somewat too dark.

### Diagnostic: Show luminance values

Let's look at how the luminance is calculated for each test color, using `grDevices::col2rgb()`
...
Then check this out using my `test_colors`:
...
In this, I was surprised that I could not find function in the `colorspace` package to do this `get_luminance()` more simply. Searching on the web, I found `chroma::luminance()` but this is not available on CRAN, and I couldn't take this on as a dependence.
