...
After describing its features, I use an example of a two-group $T^2$ test to illustrate
...
two-sided alternative, $\mathcal{H}_1 : \mu \ne \mu_0$ is
...
and the hypothesized means $\boldsymbol{\mu}_0$, in the metric of $\mathbf{S}$, as shown in @fig-T2-diagram.
\ixmain{Mahalanobis distance}
...
\ix{linear combination}

* **Eigenvalue** : Hotelling showed that $T^2$ is the one non-zero eigenvalue (latent root)
$\lambda$
of the matrix $\mathbf{Q}_H = N (\bar{\mathbf{x}} - \boldsymbol{\mu}_0)^\mathsf{T}  (\bar{\mathbf{x}} - \boldsymbol{\mu}_0)$ relative to $\mathbf{Q}_E = \mathbf{S}$. The "relative to" means that the eigenvalue problem is expressed as
...
$\mathbf{a} = \mathbf{S}^{-1} (\bar{\mathbf{x}} - \boldsymbol{\mu}_0)$. These are the 
(raw) _discriminant coefficients_, giving the relative contribution of each variable to $T^2$.
\ix{eigenvector}
\ix{discriminant analysis!coefficients}
...
[^exact]: An **exact test** in statistics is a hypothesis test that gives a true, accurate significance level ($p$-value) without relying on approximations or large-sample assumptions. For example, the complete sampling distribution of the
statistic $t = (\bar{x}_1 - \bar{x}_2) / s_p$ 
for a two-sample $t$ test can be generated in a _permutation test_ that generates all possible random
assignments of $n_1$ and $n_2$ observations to the two groups.  This trades computation for the use of any assumptions.
...
\ix{invariance!linear transformation}
...
for both variables, $\mathcal{H}_0: \mu_1 = \mu_2$, or, equivalently, that the difference
between means (a contrast!) is zero: $\mathcal{H}_0: \mu_1 - \mu_2 = \mathbf{0}$.
...
`r func("Hotelling::hotelling.test()")` [@R-Hotelling] or `r func("car::Anova()")`, but the latter is more generally useful because
...
If you want to just extract the $t$-tests, here's a handy trick using `r func("broom::tidy.mlm()")`,
...
ellipses, using `r func("heplots::covEllipses()")`. (Setting `pooled=FALSE` here omits drawing the
ellipse for the pooled covariance matrix $\mathbf{S}_p$.)
\ix{data ellipse}

<!-- fig.code: R/mathscore/mathscore-figs.R -->
...
Visually, imagine sliding the separate data ellipses to the grand mean,
...
In @sec-eqcov we take up the topic of visualizing tests of this assumption, based on 
Box's $M$-test.
...
The function `r func("heplots::heplot()")` takes the model object, extracts the $\mathbf{H}$ and $\mathbf{E}$
...
in a single figure. In @fig-mathscore-HE-overlay below I also plot the scores on the discriminant axis, by using this small function `r func("project_on()")`
...
Then, I run the same code as before (for @fig-mathscore-cov2) to plot the data ellipses, and follow this with a call
to `heplot()` using the option `add=TRUE` which adds to an existing plot. Following this,
I find the group means and draw lines projecting the points on the line between them.
```{r}
...
function `lda()`:
\ix{discriminant analysis!linear}
...
\ix{discriminant analysis!scores} 
...
relative to the two individual variables---the groups are much further apart on the discriminant axis.

\ixoff{discriminant analysis}
...
2. the data points, but they are jittered horizontally using `r func("geom_jitter()")` because otherwise they would all overlap on the X axis;
...
\ix{multivariate test statistics}
\ix{Pillai trace}
\ix{Wilks lambda}
...
from the `"mlm"` object using `r func("broom::tidy.mlm()")`.[^unistats]
What is given as the `estimate` is the difference in the mean for the genuine banknotes relative
to the counterfeit ones.

[^unistats]: The function `r func("heplots::uniStats()")` does this more compactly and displays the $R^2$ and $F$ statistics for
...
The function `r func("heplots::etasq()")` gives $\eta^2 = 0.92$, or 92% of the total variance.[^etasq]
\ix{effect size}

[^etasq]: `r func("heplots::etasq()")` calculates overall $\eta^2$, across _all_ response variables, for each term in a MLM.
You can obtain these values for each of the response variables separately using `r func("effectsize::eta_squared()")`.
...
#| Below: Observations projected on the line joining the means give discriminant scores which correspond to a one-dimensional canonical space, represented by a boxplot of their scores and arrows reflecting the variable weights."
