...
that is shown in this space. This means that two variable vectors that point in the same direction are highly correlated, $r = 1$ if they are completely aligned.
...
where experimental measures of
similarity or dissimilarity of psychological objects (colors, faces, words, Morse code symbols)
could be obtained by direct ratings, confusions, or other tasks. 
MDS provided a coherent method to study the dimensions of perceptual and cognitive space
[@Shepard-etal-1972a;@Shepard-etal-1972b].

To perform nMDS, you need to calculate the matrix of distances between all pairs of observations 
(`dist()`). The basic function is `MASS::isoMDS()`. The **vegan** package [@R-vegan]
provides `vegan::metaMDS()` which allows a wide range of distance measures 

and then call `MASS::isoMDS()`, specifying the number of dimensions (`k`) desired. 
...
distribution with one degree of freedom, rather than the Gaussian distribution for the points
...
PCA (@fig-diabetes-ggbiplot), MDS (@fig-diabetes-mds), and t-SNE ().
