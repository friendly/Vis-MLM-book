...
$\widehat{\mathbf{M}}_k$ using $k$ dimensions can be obtained as $\widehat{\mathbf{M}}_k = \mathbf{M}\;\mathbf{V}_k^T$ where $\mathbf{V}_k$ is the eigenvectors corresponding to the $k$
largest eigenvalues. The function `approx_pca()` does this, and also undoes the scaling
and centering carried out in PCA.

**TODO**: This description isn't quite right. Also, separate approximation from the pivot_longer code...
...
In numbers, the original $640 \times 955) image is of size 600 Kb. The 100 component version is 93 Kb,
15.6% of this.
