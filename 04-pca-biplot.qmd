...
## Looking ahead
I'm using the term _multivariate juicer_ here to refer the wider class of **dimension reduction** techniques,  used for various purposes in data analysis and visualization. Principal components analysis is the simplest example and illustrates the general ideas.
...
::: {.callout-warning title="Rownames"}
...
variables in the same space, by what amounts to regressions of these new variables on the principal component dimensions. For example, the left panel of @fig-supp-regession depicts the vector geometry of a regression 
...
of two principal component axes.
...
a `quanti.sup` component containing the coordinates for the supplementary variables.
This can be plotted using `FactoMiner::plot()`
...
$\widehat{\mathbf{M}}_k$ using $k$ dimensions can be obtained as $\widehat{\mathbf{M}}_k = \mathbf{X}_k\;\mathbf{V}_k^T$ where $\mathbf{X}_k$ are the principal component scores and
