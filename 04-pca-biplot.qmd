...
**TODO**: Rename Dimension Reduction
...
(1) the lengths of the variable vectors, $||\mathbf{v}_i|| = \sqrt{\Sigma_{j = 1}^{j=2} \; v_{ij}^2}$ give the proportion of variance of each variable accounted for in a two-dimensional display.
...
The brake-through for nonlinear methods came from Roger Shepard and William Kruskal
...
whose fitted distances, $\hat{d}_{ij} = ||\mathbf{x}_i - \mathbf{x}_j||$ matches
the order of the $d_{ij}$ as closely as possible.
The impetus for this stemmed largely from psychology, where experimental measures of
similarity or dissimilarity of psychological objects (faces, words, Morse code symbols)
could be obtained by direct ratings, confusions, or other tasks and MDS provided
a method to study the dimensions of perceptual and cognitive space.

To perform nMDS, calculate the matrix of distances between all pairs of observations and
call `MASS::isoMDS()`. It returns the coordinates in a dataset called `points`.
...
To plot the solution, I'll use `ggpubr::ggscatter()` here because it provides concentration ellipses 
...
correlations between variables and dimensions from @sec-supp-vars, I do this as follows:

```{r}
#| label: fig-diabetes-mds
#| out-width: "100%"
#| fig-cap: "Nonmetric MDS representation of the Diabetes data."
