...
1. The last line follows because $\mathbf{\Lambda}$ is a diagonal matrix, so $\mathbf{S}$ is expressed as a sum of outer products of each $\mathbf{v}_i$ with itself, times the eigenvalue $\lambda_i$. 

2. The columns of $\mathbf{V}$ are the eigenvectors of $\mathbf{S}$. They are orthogonal
and of unit length, so $\mathbf{V}^\mathsf{T} \mathbf{V} = \mathbf{I}$ and thus
...
7. Here is a key fact: If, as usual, the eigenvalues are arranged in order, so that $\lambda_1 > \lambda_2 > \dots > \lambda_p$, then the first $d$ components give a $d$-dimensional approximation to $\mathbf{S}$, which accounts for $\Sigma_i^d \lambda_i$ of the $\Sigma_i^p \lambda_i$ total variance, usually interpreted as the proportion,
...
($n = 10$) of workers in a factory. The dataset `matlib::workers` contains these and other variables.
...
when the variables are measured on different scales[^pca-scales], unless you want to
...
in most cases the default is OK. 
...
This fact prompted the rough _rule of thumb_ to extract principal componends whose eigenvalues exceed 1.0,
...
The `r package("adegraphics", cite=TRUE)` produces lovely biplots using `r pkg("lattice")`
...
of the Mahalanobis $D^2$ values, three Penguins (283,  10,  35) were identified as noteworthy, deserving a closer look to see why they are unusual. It was pointed out that 2D plots of the data variables were only partially
revealing.
Let's see where they appear in biplots.

First, I find the noteworthy points as before:
...
The PCA shows that the first two components accounts for 88% of variance:
```{r peng-pca}
...
#| out-width: "60%"
#| fig-cap: "Biplot of the first two dimensions of the Penguin data"
...
a contrast of bill length with bill depth and body mass. The Chinstraps here have the longest, straightest
beaks. Our friend Cyrano (case 283), which is the only true multivariate outlier, lies at the extreme ends of
both dimensions. Case 10 (Hook Nose) stands out at the high end of dimension 3, while case 35 is at the high
end of dimension 4.

```{r}
#| label: fig-peng-out-biplot2
#| out-width: "60%"
#| fig-cap: "Biplot of dimensions 3-4 of the Penguin data"
