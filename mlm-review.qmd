...
$$
\sizedmat{Y}{n \times p} = 
\sizedmat{X}{n \times q} \, \sizedmat{B}{q \times p} + \sizedmat{U}{n \times p} \:\: ,
$$
...
### Assumptions
The assumptions of the multivariate linear model entirely concern the behavior of the errors (residuals). 
Let $\mathbf{u}_{i}^{\prime}$ represent the $i$th row of $\mathbf{U}$. Then it is assumed that 

* The residuals, $\mathbf{u}_{i}^{\prime}$ are distributed as multivariate normal, 
