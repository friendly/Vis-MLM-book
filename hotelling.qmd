...
squared Mahalanobis distance between the sample mean vector $(\bar{x}_{BM}, \bar{x}_{WP})^\textsf{T}$
...
$\lambda$
of the matrix $\mathbf{Q}_H = N (\bar{\mathbf{x}} - \mathbf{\mu}_0)^\textsf{T}  (\bar{\mathbf{x}} - \mathbf{\mu}_0)$ relative to $\mathbf{Q}_E = \mathbf{S}$
