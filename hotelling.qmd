...
is assessed relative to the pooled within-group covariance matrix $\mathbf{S}_p$, which is just a size-weighted average to the two within-sample matrices, $\mathbf{S}_1$ and $\mathbf{S}_2$,
...
Discriminant analysis **CIT?** for two-group designs or for one-way MANOVA
...
The coefficients give $w = -0.84 \text{BM} + 0.75 \text{WP}$.  This is exactly the direction given by the line for the $mathbf{H}$ ellipse in the figure REF

To round this out, we can calculate the discriminant scores by multiplying the matrix $\mathbf{X}$
by the vector ??? of the discriminant weights.
...
Then a $t$-test on these scores gives Hotelling's $T$
...
the equivalent $F$ statistic found by `Anova()` is 28.9. Verify that @eq-Fstat gives this
result.
