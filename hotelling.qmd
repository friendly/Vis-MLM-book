...
A classic example is the dataset `mbclust::banknote`, containing six size measures made on 100 genuine and 100 counterfeit old-Swiss 1000-franc bank notes [@FleuryReidwyl-1988]. The goal is to see how well the real and fake
...
Before considering hypothesis tests, let's look at some exploratory graphics. @fig-banknote-violin shows univariate violin  and boxplots of each of the measures. A quick glance shows that the counterfeit and genuine bills differ in their
...
Bivariate relations could be explored in data space using scatterplots, I turn to my trusty multivariate juicer, a biplot.
...
The biplot in @fig-banknote-biplot gives a nice summary, at least in two dimensions. 
The first component shows the positive correlations among the measures of the margins, where the 
counterfeit bills are larger than the real ones and a negative correlation of the Diagonal with these.
The length of bills only distinguishes the types of banknotes on the second dimension.
```{r}
#| label: fig-banknote-biplot
#| out-width: "100%"
#| fig-cap: "Biplot of the banknote variables ..."
...
If you wish, you can extract the univariate $t$-tests from the `"mlm"` object using `broom::tidy.mlm()`.
...
and $\mathbf{Q}_T = \mathbf{Q}_H + \mathbf{Q}_E$, and there are different different calculations for a single
measure corresponding to the various test statistics (Wilks' $\Lambda$, etc.), as described in **Chapter XX**.
...
This can be compared (unfavorably) to the principal components analysis and the biplot in @fig-banknote-biplot,
where two components accounted for 70% of total variance and it took four PCA dimensions to account for over 90%.
The goals of PCA and MANOVA are different, of course, but they are both concerned with accounting for variance
of multivariate data. We will meet another multivariate juicer, **canonical discriminant analysis** in **Chapter XX**.
