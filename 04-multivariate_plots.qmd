...
Inhabitants of Spaceland would recognize the algorithm immediately: Take that Unit Sphere, squeeze her through
the Cholesky machine, then expand or shrink to cover a certain percentage of the data points
as, and then just shift her center to the means.

**Contour levels ($c$)**

There is an easy way to callibrate the size or scale factor, $c$ of the data ellipse. 
...
Statistical ellipses are conveniently drawn using `car::ellipse()`. `heplots::ellipse.label()` provides flexible
...
and we can see the greater variance for $\mathbf{y}_2$ as well as the correlation. Facilities of `grDevices::plotmath()`
...
* `ggplot2::stat_ellipse()`: uses the calculation methods of `car::dataEllipse()` to add unfilled (`geom = "path"`) or filled (`geom = polygon"`) data ellipses in a `ggplot2` scatterplot, using inherited aesthetics.
