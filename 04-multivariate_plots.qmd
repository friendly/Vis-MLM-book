...
An important example is **Simpson's paradox** [@Simpson:51] which occurs when the marginal and
...
**TODO**: Decide how to handle the other plots
**(a) Ignoring species**
...
about 1% or 9 to have larger squared distances than this cutoff. (Normally, allowing for
the fact that we are looking at the largest values in a sample of size $n$, we would use
a much smaller individual significance level, say $\alpha = 0.001$ or smaller still.)
...
The pointwise 95% confidence envelope is calculated as $D^2_{(i)} \pm 1.96 \times \text{se} ( D^2_{(i)} )$ where the standard error is calculated [@Chambers-etal:83, Sec. 8.6] as
...
$d(q_i)$ is the density of the chi square distribution at the quantile $q_i$ and $p_i$ is
...
Two cases (10, 283) appear to be very unusual in this plot, in relation to other members of their species. But only case 283 is a true multivariate outlier, as shown in the cqplot
...
It bears noting that for linear models, multivariate normality is **not required** for the response variables
or predictors, but rather is an assumption for the _residuals_ from a model. As well, multivariate
...
were highly non-normal or showed outliers.
