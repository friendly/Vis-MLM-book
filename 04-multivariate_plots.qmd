...
for plots below. I also define a convenience function `legend_inside()` to make it less verbose to position a legend _inside_ a plot window,
...
(e.g., `labels`, `theme_penguins()`) once, and then re-use
...
This is then used to subset the data for `geom_text()` that writes the case id numbers.
