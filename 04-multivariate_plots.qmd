...
As we saw with Anscombe's quartet (@sec-anscombe) and the Davis weight data (@sec-davis)
numerical summaries hide features that are immediately apparent in a plot.
\ix{quartet!Anscombe}
...
scatterplot with graphical _annotations_ such as fitted lines, curves (@sec-bivariate_summaries) and data
ellipses (@sec-data-ellipse) which serve to add _visual summaries_ of the relation between two variables.

To visualize more than two variables, we can view all pairs of variables
in a scatterplot matrix (@sec-scatmat)
or shift gears entirely to show multiple
variables along a set of parallel axes (@sec-parcoord). These are in the category I call _reconnasiance plots_,
designed to give a high-level overview of multivariate data.

As the number of variables
increases, we may need to suppress details with stronger summaries (@sec-visual-thinning)
...
Another approach to visualizing high-D data uses _animated tours_ (@sec-animated-tours) to show the data in a sequence
of low-D projections, typically 2D, designed to reveal interesting features that might not otherwise be visible.
At a higher level of abstraction, _network diagrams_ (@sec-network) representing the structure of correlations among a possibly large set of variables offer another useful technique.
...
The basic scatterplot is the workhorse of multivariate data visualization, showing how one variable, $y$, often an outcome to be
explained by or varies with another, $x$. It is a building block for many useful techniques, so it is helpful to understand how it can be
...
anomalies in compensation. The dataset `r dataset("carData::Salaries")` gives data on
...
we would want to consider faculty `rank` (related also to `yrs.service`)
and `discipline`, recorded here as `"A"` ("theoretical" departments) or
`"B"` ("applied" departments). But, for a basic plot, I will ignore these
...
#| out-width: "80%"
#| fig-cap: "Naive scatterplot of Salary vs. years since PhD, ignoring other variables, and without graphical annotations other than a rug plot of the marginal distributions."
gg1 <- ggplot(Salaries, 
...
-   Data beyond 50 years is thin, but there are some quite low salaries
    ...
    _marginal distributions_ of the observations. Jitter and transparency helps to avoid overplotting
...
"sees" in the data. Statistical tests <!--# (secref?) --> can help you decide if there is more evidence for a quadratic fit compared to the simpler linear relation. 
...
These can be of two types: _pointwise intervals_ or _simultaneous intervals_.
The default setting in `r func("ggplot2::geom_smooth()")` calculates pointwise intervals
(using `stats::predict.lm(..., interval="confidence")` at a confidence level $1-\alpha$ for the predicted response at _each value_ $x_i$ of a predictor, and have the frequentist interpretation that over repeated sampling only $100\;\alpha$ of the predictions at $x_i$ will be outside that interval. 
    In contrast, simultaneous intervals are calculated so that $1 - \alpha$ is the probability that _all of them_ cover their corresponding true values simultaneously. These are necessarily wider than pointwise intervals. Commonly used methods for constructing simultaneous confidence bands in regression are the Bonferroni and Scheffé methods, which control the family-wise error rate over all values of $x_i$. See [](https://en.wikipedia.org/wiki/Confidence_and_prediction_bands) for precise definitions of these terms.
...
\ix{confidence!band}
\ix{confidence!pointwise}


#### Non-parametric smoothers {.unnumbered}
\ix{smoother!non-parametric}
\ix{loess}
\ix{lowess}
...
statistical models such effects are *interaction* terms, represented by `:` in a model term, as in
`y ~ group + x + group:x`. The term `group:x` fits a different
slope for each group and the grouping variable is often called a
*moderator* variable, because it  alters the strength or direction of the relationship of `x` to `y`.
Common moderator variables are ethnicity, health
status, social class and level of education. Moderators can also be
continuous variables as in `y ~ x1 + x2 + x1:x2`. You can read this to
say that the slope for `x1` varies with the value of `x2`.

In plots, I call these *stratifiers*, suggesting that we should consider how
the overall relation changes over such
"other" variables. Such variables are most often factors, but we can cut
a continuous variable into possibly overlapping ranges (*shingles*) and do the same
graphically. There are two general graphical techniques for stratifying:
\ix{shingle}
...
-   **Grouping**: Identify subgroups in the data by assigning different
    ...
    single plot, as shown in @fig-Salaries-rank below. This is quite natural for factors; quantitative
...
-   **Conditioning**: Showing subgroups in different plot panels, as in @fig-Salaries-faceted. This
...
::: {.callout-note title="History Corner: Model Syntax---Coplots and faceting"}
The syntax you use in specifying a plot plays an important role
in visual thinking. Ideally, you want the shortest path between
having a graphic idea in your head and specifying that in code to see the result rendered on your screen.
In the same way, you should appreciate the conceptual simplicity of the syntax to specifying a model in R.

The @WilkinsonRogers1973 model notation, `y ~ x1 + x2 + ...` was a computational equivalent to the 
analogous statistical model, $y = \beta_0 + \beta_1 x_1 +  \beta_2 x_2 + \dots$. For factors, the notation,
`y ~ A + B + A*B + ...`, was a totally natural way to say: "Fit a two-way ANOVA to factors A and B,
allowing for a possible interaction." 

Importantly, this notation provided the impetus and 
conceptual ideas that lead to implementation in the GLIM program [@Nelder1974glim], which
simultaneously extended model thinking by incorporating the ideas and theory of generalized linear models
[@NelderWedderburn1972GLM] as natural extensions of the classical OLS linear model to encompass
non-normal error models (logistic regression, Poisson count data models), using a single
algorithm (Iteratively Reweighted Least Squares) to fit various models.
\ix{visual thinking}

In plotting data and models, the
recognition of the roles of visual grouping by factors within a panel
and conditioning in multi-panel displays was a similarly important advance in the
development of modern statistical graphics software. It began at A.T.& T. Bell
Labs in Murray Hill, NJ in conjunction with the **S** language, the
mother of R. 

The "S Book" [@ChambersHastie1991] laid out the framework for fitting models with `lm()`
and `glm()`, the notion of model objects and methods like `plot()` and `coef()` for working with them,
and the basic ideas of base-R graphics, such as adding `text()`, `lines()` and `points()` to a plot.
...
-   A **graphing syntax** similar to that used in statistical model
    formulas: `y ~ x | g` conditions the data by the levels of `g`, with the
    `"|"` operation read as "given"; two or more conditioning variables are specified as
...
-   **Panel functions** define what is plotted in a given panel.
    ...
    `r func("latticeExtra::panel.smoother()")` for loess smooths and a wide
    variety of others. Panel functions represent a different way of thinking about how to produce a graph.

The `r package("car", cite=TRUE)` supports this graphing syntax, `y ~ x | g`, in many of
its functions. 
However `r pkg("ggplot2")` does not; instead, it uses aesthetics (`aes()`), which
map variables in the data to visual characteristics in displays for visual grouping,
and "faceting" for conditioning a single plot into separate panel, motivated by the 
_Grammar of Graphics_ [@Wilkinson:99].

The `y ~ x | g` notation is now widely used in model formulas for a variety of cases where this notation makes
sense and eases model thinking:

- Mixed-effect models: `(1 | group)` for a random intercept, fit by `r pkg("lme4")`, which provide linear (`lmer()`) and generalized linear (`glmer()`) mixed models; 
- Nonlinear mixed-effect models: `r pkg("nlme")` uses a similar syntax, `random = ~1 | group` to specify the random effects explicitly.
- Bayesian mixed models: `(1 | group)`, fit by `r pkg("brms")`.
\ix{mixed models}

This extended formula notation has been generalized in the `r package("Formula", cite=TRUE)`.
This notation accommodates instrumental variables models, like `y ~ x1 + x2 | z1 + z2 + z3`
(fit with `r pkg("ivreg")`), hurdle and zero-inflated count data models,
like `y ~ x1 + x2 | x1 + x3`, where there is one model for the positive counts and a separate model for
the zero counts (fit with `r pkg("pscl")`) and models with multiple outcome variables,
like `y1 + y2 ~ x1 + x2 + x3`. It is a shame that this notation is not yet implemented for MLMs,
so we are stuck with `cbind(y1, y2) ~ x1 + x2 + x3`.

:::

### Using grouping

For the `Salaries` data, the most obvious variable that affects academic salary is `rank`,
...
differentiated by color. In the case of `r func("geom_smooth()")`, we get a
...
applied disciplines earn about \$10,000 more per year on
...
There are two options: `r func("facet_wrap()")` takes one or more conditioning
...
The data ellipse is typically used to add a visual summary to a scatterplot---one
that shows _all together_ the means, standard deviations, correlation,
...
See @Friendly-etal:ellipses:2013 for a complete discussion of the lovely role of
ellipsoids in statistical data visualization and thinking.

The data ellipse is based on the idea that in a bivariate normal distribution, the contours
...
#| fig-align: center
#| out-width: "50%"
#| fig-cap: "Which of the points **A** and **B** is further from the mean (X)? The plot shows 2D data points with curves of constant 'distance' from the centroid. The blue solid ellipse shows a contour of constant Mahalanobis distance, taking the correlation into account; the dashed red circle is a contour of equal Euclidean distance. Given the data points,   _Source_: Re-drawn from [Ou Zhang](https://ouzhang.rbind.io/2020/11/16/outliers-part4/)"
...
#| fig-cap: !expr glue::glue("2D data with curves of constant distance from the centroid. The {blue} solid ellipse shows a contour of constant Mahalanobis distance, taking the correlation into account; the dashed {red} circle is a contour of equal Euclidean distance. Given the data points,  which of the points **A** and **B** is further from the mean (X)? _Source_: Re-drawn from [Ou Zhang](https://ouzhang.rbind.io/2020/11/16/outliers-part4/)")
...
Which is further from the mean, "X"? Assuming the data to be uncorrelated,
the `r colorize("red")` dashed circle shows a contour of constant Euclidean distance.
This ignores the apparent negative correlation, so point "A" is much further by this metric.
...
Mathematically, Euclidean (squared) distance ($D_E^2(y)$) for $p$ variables, $y_j = y_1, y_2, \dots , y_p$,
...
$$
where $\mathbf{S} = ({n-1})^{-1} \sum_{i=1}^n (\mathbf{y}_i - \bar{\mathbf{y}})^\textsf{T} (\mathbf{y}_i - \bar{\mathbf{y}})$ is the sample variance-covariance matrix.
...
by the variances **and covariances** of the variables, generalizing the univariate case.
...
### Drawing data ellipses

A computational definition of the data ellipsoid recognizes that the boundary of an ellipsoid can be found by (a)
starting with a unit sphere $\mathcal{P}$
centered at the origin, (b) transforming that by a "square root" of the covariance matrix, denoted $\mathbf{S}^{1/2}$,
and then (c) shifting that to centroid of the data. 

The unit sphere is defined as the contour of Euclidean distance 1 from the origin,
$\mathcal{P} := \{ \mathbf{x}^\textsf{T} \mathbf{x}= 1\}$.
Using the notation $\oplus$ to represent translation to a new centroid at the variable means, $\bar{\mathbf{y}}$,
the data ellipsoid becomes,

$$
\mathcal{E}_c (\bar{\mathbf{y}}, \mathbf{S}) = \bar{\mathbf{y}} \; \oplus \; \mathbf{S}^{1/2} \, \mathcal{P} \period
$$
The matrix $\mathbf{S}^{1/2}$ represents a rotation and scaling of the sphere
and is commonly computed as the Cholesky factor of $\mathbf{S}$ given in R by `chol(S)`. You
can imagine this in 2D by thinking of how the dashed `r red` circle in @fig-mahalanobis could be transformed
into the `r blue` ellipse.
...
Inhabitants of Spaceland would recognize the algorithm immediately: Take that Unit Sphere $\mathcal{P}$, squeeze her through the Cholesky machine giving $\sqrt{\mathbf{S}}$, then expand or shrink to cover a certain percentage of the data points
as, and then just shift her center to the means.

#### Contour levels ($c$) {.unnumbered}

There is an easy way to callibrate the size or scale factor, $c$ of the data ellipse to cover a given
...
($\chi^2$ with 2 df),
so ellipses of various conventional sizes can be calculated using `qchisq()`:
...
* $c^2 = \chi^2_2 (0.68) = 2.28$ gives a "1 standard deviation bivariate ellipse,"
an analog of the standard interval $\bar{y} \pm 1 s$,
* $c^2 = \chi^2_2 (0.95) = 5.99 \approx 6$ gives a data ellipse of 95\% coverage.

In not-large samples, the radius $c$ of the ellipsoid is better approximated by a multiple of a $F_{p, n-p}$ distribution, with $p$ variables and $n-p$ degrees of freedom for $\mathbf{S}$.
This gives a radius $c =\sqrt{ 2 F_{2, n-2}^{1-\alpha} }$
in the bivariate case ($p=2$) for coverage $1-\alpha$.

These three conventional cases are illustrated in @fig-ellipses-coverage with ellipses of 50%, 68% and 95% coverage for a the matrix $\mathbf{S}$
defined below and $\bar{\mathbf{y}} = \mathbf{0}$. Here, the variance of $\mathbf{y}_1$ is twice that of $\mathbf{y}_2$ 
and the correlation works out to $r(y_1, y_2) = 0.35$.
...
<!-- fig.code: R/ellipses-coverage.R -->

Statistical ellipses are conveniently drawn using `r func("car::ellipse()")` or  `r func("ggplot2::stat_ellipse()")`.
`r func("heplots::ellipse.label()")` provides flexible
ways to add labels to ellipses at various locations around the ellipse shown in @fig-ellipses-coverage.
These are called repeatedly to overlay the three ellipses.
...
#| fig-height: 7
#| out-width: "70%"
#| fig-cap: "Data ellipses of 50%, 68% and 95% coverage when the means are $\\bar{\\mathbf{y}} = \\mathbf{0}$ and
...
As always, graphic details matter. 
@fig-ellipses-coverage uses `asp = 1` so that units in the plot are the same for $\mathbf{y}_1$ and $\mathbf{y}_2$,
and we can see the greater variance for $\mathbf{y}_2$ as well as the correlation. Facilities of `r func("grDevices::plotmath()")`
are used to provide mathematical annotations in the plot.
...
To complete the picture, he added lines showing the geometric major and minor axes of the
family of ellipses (which turned out to be the principal components) with the result shown in @fig-galton-corr.
\ix{principal components}
\ix{ellipses}
...
You can see the following properties of a data ellipse in @fig-galton-ellipse-r.
...
* The regression line predicting $y$ from $x$ goes through the points where the ellipses have vertical tangents to the ellipse. The _other_ regression line, predicting $x$ from $y$ goes through the points of horizontal
tangency.
...
Because Galton's values of `parent` and `child` height were recorded in discrete class intervals of 1 in.,
...
`r func("car::dataEllipse()")` for the ellipses.
...
square roots, $\sqrt{\lambda_1}, \sqrt{\lambda_2}, \dots$ of the corresponding
eigenvalues.
...
* `r func("car::scatterplot()")`: uses base R graphics to draw 2D scatterplots, with a wide variety of plot enhancements including linear and non-parametric smoothers (loess, gam), a formula method, e.g., `y ~ x | group`, and marking points and lines using symbol shape,
color, etc. 
    Importantly, the `r pkg("car")` package generally allows automatic identification of "noteworthy" points by their labels in the plot using a variety of methods. For example, `method = "mahal"` labels cases with the most extreme Mahalanobis distances;
`method = "r"` selects points according to their value of `abs(y)`, which is appropriate in residual plots.
* `r func("car::dataEllipse()")`: plots  classical or robust data ellipses for one or more groups, with the same facilities for point identification. The robust version (`robust=TRUE`) uses the multivariate $t$ distribution (using `MASS::cov/trob()`) rather than the Gaussian.
* `r func("heplots::covEllipses()")`: draws classical or robust data ellipses for one or more groups in a one-way design and optionally for the pooled total sample, where the focus is on homogeneity of within-group covariance matrices.
* `r func("ggplot2::stat_ellipse()")`: uses the calculation methods of `r func("car::dataEllipse()")` to add unfilled (`geom = "path"`) or filled (`geom = polygon"`) data ellipses in a `ggplot2` scatterplot, using inherited aesthetics.
...
1971 Canadian Census, recorded in `r dataset("Prestige")`.[^prestige-src]
...
<!-- fig.code: R/prestige/prestige.R -->
...
#| out-width: "70%"
#| fig-cap: !expr glue::glue("Scatterplot of prestige vs. income, showing the linear regression line ({red}), the loess smooth with a confidence envelope ({darkgreen}) and a 68% data ellipse. Points with the 4 largest Mahalanobis $D^2$ values are labeled.")
par(mar = c(4,4,1,1)+.1, cex.lab = 1.2)
...
* The bivariate 1 standard deviation
data ellipse, centered at the means encloses approximately 68% of the data points. It adds visual information about the correlation and precision of the linear regression; but here, the non-linear trend for higher incomes strongly suggests a different approach.
...
### Handling nonlinearity: Plotting on a log scale {#sec-log-scale}
...
says that a $1000 increase in `income` is associated with an average
...
increase in salary is associated with an average increase of 21.55 / 100 in prestige.
in prestige. 
In general, when we fit a linear model  $y \sim \ln(x)$, a $P$\% change in $x$
the expected change in $y$ is $b \cdot \ln(1 + P/100)$.
...
#| out-width: "70%"
#| fig-cap: "Scatterplot of prestige vs. income, stratified by occupational type. This implies a different interpretation, where occupation type is a moderator variable, so different regressions apply to each type. The linear regression line is dashed, while the loess smooth is solid."
par(mar = c(4,4,1,1)+.1)
...
higher occupational category. A different fitted relationship entails a different story.
...
The `penguins` dataset, originally from the `r package("palmerpenguins", cite=TRUE)` provides further instructive examples of plots and analyses of multivariate data. The data consists of measurements of body size 
...
#| fig-align: center
#| out-width: "50%"
#| fig-cap: "Penguin species observed in the Palmer Archipelago. This is a cartoon, but it illustrates some features of penguin body size measurements, and the colors typically used for species.  Image credit: Allison Horst"
...
particularly those between the sexes (sexual dimorphism) in relation to foraging and habitat.

Here, I use a slightly altered version of the dataset, `r dataset("heplots::peng")`,
constructed by renaming variables to remove the units,
making factors of character variables and deleting a few cases with missing data.[^penguins]

[^penguins]: In R 4.5.0, a revised version of the `palmerpenguins` dataset, named `r dataset("penguins")`
was added to the base R `r package("datasets")`. This corrected a few of the problems that led me to create
`r dataset("heplots::peng")`, but still retains the cases with missing data.
...
#| echo: false
#| out-width: "50%"
#| fig-cap: "Color palettes used for penguin species. Each species has a primary hue, which can be used in
#|       light, medium or dark versions."
...
This is used to define a `r func("theme_penguins()")` function that I use to simply change the color and fill scales
for plots below. I also define a convenience function `r func("legend_inside()")` to make it less verbose to position a legend _inside_ a plot window,
because outside legends reduce resolution within the plot.
...
### Visual thinning {#sec-visual-thinning}
\ixon{visual thinning}
...
In @fig-peng-ggplot2 I chose to include only the regression line and
...
#| out-width: "70%"
#| code-fold: show
#| fig-cap: "**Bagplot**: For each Penguin species the darker inner (bag) polygon reflects the innermost 50% of the data points. The outer (fence) polygon corresponds to points enclosed by a multiple of the bag. Points outside the fence for each species are plotted individually."
ggplot(peng, 
       aes(x = bill_length, y = bill_depth,
           color = species, shape = species, fill=species)) +
  geom_smooth(method = "lm", formula = y ~ x,
...
## Non-parametric bivariate density plots {#sec-bivar-density}

\ixon{non-parametric!bivariate density plot}
\ixon{bivariate density plot}
...
To see more detail about the "shape" of bivariate data in a non-parametric way
you can use one of the methods described below to see a model-free representation
of your data.

For a single variable, `r func("stats::density()")` and `r func("ggplot2::geom_density()")`
...
For bivariate data,
`r func("MASS::kde2d()")` estimates the density on a square $n \times n$ grid over the
ranges of the variables.

`ggplot2` provides `r func("geom_density_2d()")` which uses `r func("MASS::kde2d()")` and displays these as contours---
horizontal slices of the 3D surface at equally-spaced heights and projects these onto the 2D plane.
The `r package("ggdensity", cite=TRUE)` extends this with `r func("geom_hdr()")`,
...
\ix{high density region}

@fig-peng-ggdensity shows these side-by-side for comparison.
With `r func("geom_density_2d()")` you can specify either the number of contour `bins` or the
width of these bins (`binwidth`). For `r func("geom_hdr()")`, the `probs` argument gives a result that
...
Compared with the data ellipse, which is highly smoothed by the Gaussian assumption and the bagplot,
which is smoothed by the concept of contours of data depth, the plots in @fig-peng-ggdensity show considerably
more detail, and an intriguing suggestion of two peaks for our Chinstrap penguins.
\ixoff{non-parametric!bivariate density plot}
\ixoff{bivariate density plot}

## Simpson's paradox: Marginal and conditional relationships {#sec-simpsons}

\ixon{Simpson's paradox}
...
One class of simple, but important, examples concerns the difference between the _marginal_ relationship between variables, ignoring some important factor or covariate, and the _conditional_ relationship, adjusting (controlling) for that variable.

An important example of this is **Simpson's paradox** [@Simpson:51] which occurs when the marginal and
conditional relationships differ in _direction_. That is, the overall correlation
...
For Flatlanders in 2D space, this is a puzzlement.

We can see this paradox in the plots
...
differences among species, is shown in panel (c) of @fig-peng-simpsons.
...
<!-- **TODO** This panel tabset looks fine in HTML but is awkward in PDF [FIXED] -->
...
(e.g., `labels`, `r func("theme_penguins()")`) once, and then re-use
...
<!-- **TODO**: Decide how to handle the other plots: just mention them, don't show code [FIXED] -->

The `ggplot2` code for panel (a), ignoring species, is shown below. The by-species plot (b)
simply maps aesthetics for color, fill and shape to `species`. The within-species plot (c)
centers the data for each group at the grand means.
...
\ixoff{Simpson's paradox}

<!-- **TODO**: Add stuff on 3D scatterplots, using the `R/penguin/peng-3D-rgl.R` example. A start on this is in -->
<!-- `child/04-3D-scat.qmd` -->

## Multivariate normality and outliers {#sec-multivar-normality}

\ixon{multivariate normality}
\ixon{outliers}

The relation of the data ellipsoid for $p$ variables to the $\chi^2_p$ distribution of Mahalanobis $D^2_M (\mathbf{y})$
with
...
MVN distribution by plotting the _quantiles_[^quantile] of the _sorted_ sample Mahalanobis $D^2$ values
(denoted $D^2_{(i)}$) calculated from @eq-Dsq
against the corresponding $\chi^2_p$ quantiles found from `qchisq(df = p)`. This is called
a $\chi^2$ QQ plot.
\ix{$\chi^2$ QQ plot}

The essential idea is that the plotted points should then approximately fall along a 45$^o$ line of  slope = 1 when the data are MVN (and axes are scaled equally).
...
A variety of _robust_ methods described later (@sec-robust-estimation) work by down-weighting outliers,
which is particularly important in assessing whether the residuals from a multivariate linear
model are multivariate normal.

In this section, I focus on the $\chi^2$ QQ plot and graphical methods to relate the
...
Mahalanobis $D^2$ values are calculated by `r func("heplots::Mahalanobis()")`.
...
about 1% or 9 to have larger squared distances than this cutoff.[^adjusted] 

[^adjusted]: Often in outlier-detection problems, allowing for
the fact that we are looking at the _largest_ values in a sample of size $n$, we would use
a much smaller individual significance level, say $\alpha = 0.001$ or something like
a Bonferroni correction, $\alpha / n$.

Only three cases are identified here:
...
$\chi^2$ QQ plots are constructed by `r func("heplots::cqplot()")`.
...
The function returns (invisibly) the $D^2_{(i)}$ and corresponding $\chi^2_2$ quantile
along with the upper-tail $p$-value.
```{r}
#| label: fig-galton-cqplot
#| out-width: "70%"
#| fig-cap: "**Chi-square QQ plot** of Galton's data on heights of parents and their offspring, with a 95%
#|     pointwise confidence envelope. The three largest $D^2$ values are identified with their observation labels."
...
data are reasonably distributed as multivariate normal and also to flag potential outliers, which would appear outside the shaded
confidence region

The _pointwise_ 95% confidence envelope is calculated as $D^2_{(i)} \pm 1.96 \times \text{se} ( D^2_{(i)} )$ where the standard error is calculated [@Chambers-etal:83, Sec. 8.6] as

$$
 \text{se} ( D^2_{(i)} ) = \frac{\hat{b}}{d ( q_i)} \times \sqrt{  p_i (1-p_i) / n }  \period
$$
Here, $\hat{b}$ is an estimate of the _actual_ slope of
the reference line, obtained from the ratio of the interquartile range of the
$D^2$ values to that of the corresponding $\chi^2_p$ distribution.
$d(q_i)$ is the density of the chi square distribution at the quantile $q_i$. $p_i$ is
the corresponding $\chi^2$ percentile, so the square-root term is like a standard error for a binomial probability.
...
and made distinctive in the plot. For this plot, because the heights are recorded in
whole inches, I draw the data ellipses first without plotting the points and then
draw the points using `jitter()` on top of the data ellipses.

```{r echo = -1}
#| label: fig-galton-outliers
#| out-width: "70%"
#| fig-cap: "Scatterplot of Galton's data showing 68% and 95% data ellipses for the observations on parent and child height. The discrete points have been jittered to avoid overplotting. The three observations identified as having large $D^2$ values are labeled with their observation number and given distinctive size, color and shape."
...
them poster children for the idea of regression toward the mean.

### Penguin data {#sec-multnorm-penguin}

Let's do a similar analysis to assess multivariate normality and identify possible
outliers for the Penguin data. In the `cqplot()` shown in @fig-peng-cqplot,
I use the same point symbols and colors as in @fig-peng-ggplot1.
For illustration, I again label the three most extreme points.

```{r}
#| label: fig-peng-cqplot
#| out-width: "70%"
#| fig-cap: "Chi-square QQ plot of the Pengiun data. The three cases with the largest $D^2$ values are identified with their case numbers."
...
To relate this to the data, we can plot the data, as was done
in @fig-peng-ggplot1, and label the points identified as noteworthy. It is a bit tricky to label points selectively in `ggplot2`, when the criterion for which points
to label is complex, or involves variables outside the data frame.
Here, I create a logical variable `note`, which is `TRUE` for the noteworthy ones.
This is then used to subset the data for `r func("geom_text()")` that writes the case id numbers.
@fig-peng-ggplot-out shows the resulting plot.

```{r}
#| label: fig-peng-ggplot-out
#| out-width: "70%"
#| fig-cap: "Plot of bill length and bill depth, with the noteworthy points labeled. Only one (case 283) is a true multivariate outlier."
#| eval: true
...
Two cases (10, 283) appear to be very unusual in this plot, in relation to other members of their species. But only case 283 is a true multivariate outlier, as we saw in the cqplot example,
@fig-peng-cqplot. We could call this long-billed penguin "Cyrano".[^cyrano] 
I'll call case 10, with a very short and curved bill, "Hook Nose". 

[^cyrano]: After the long-nosed Cyrano de Bergerac from Edmund Rostand's play.
Actually, bird 283 is a female. But, it would be a mistake to call her "Rozanne" instead.

Of course, we are only looking at the data in the 2D space of the bill variables, but possible outliers
exist in  four dimensional Penguin space. Case 35 is well inside the Adélie
ellipse, so perhaps it is unusual on one of the other variables.
It turns out that multivariate outliers can most often
be easily seen as unusual observations in a projection of the data into the space of the _smallest_
principal components. I return to this example in @sec-outlier-detection.

It bears noting that for linear models, multivariate normality is **not required** for the _response_ variables
or _predictors_. Rather, this is just an assumption for the _residuals_ from a model. Multivariate
outliers in the responses may not turn out to be unusual when the predictor variables are taken into
account. In this example, a multivariate model would include the effect of `species`,

```r
peng.mod <- lm(cbind(bill_length, bill_depth, 
                     flipper_length, body_mass) ~ species, data = peng)
```
and there would be cause for concern if the residuals from this model, `residuals(peng.mod)`
were highly non-normal or showed outliers. `heplots::cqplot(peng.mod)` naturally gives the
$chi^2$ QQ plot of the residuals from this model.
...
\ixoff{multivariate normality}
\ixoff{outliers}

## Scatterplot matrices {#sec-scatmat}

\ixon{scatterplot matrix}
...
diagonal cells give labels for the variables, which serve as a guide
...
version is provided by `r func("car::scatterplotMatrix()")` which can add the
...
variable; for example, `education` appears to be
...
`r func("scales::hue_pal()")` to mimic the discrete color scale used in
`ggplot2`).
...
`r colorbox("Adelie", "orange")` penguins have shorter bill lengths than
the others, while `r colorbox("Gentoo", "green")` penguins have smaller
bill depth, but longer flippers and are heavier than
`r colorbox("Chinstrap", "purple")` and `r colorbox("Adelie", "orange")`
penguins.
...
```{r}
#| eval: false
peng.lda <- MASS::lda(species ~ cbind(bill_length, bill_depth, 
...
linear relation and the loess smooth to show potential non-linearity.
<!--# I believe you used non-linear, hyphenated earlier so I added the - --> 
To
...
#| fig-height: 8
#| out-width: "90%"
#| fig-cap: "**Visual thinning**: Scatterplot matrix of the crime data, showing only high-level summaries of the linear and nonlinear relations between each pair of variables."
crime |>
...
#| echo: false
#| out-width: "80%"
#| fig-cap: "**Corrgrams**: Some renderings for the value of a correlation in a corrgram display, conveying sign and magnitude in different ways, each encoded with aspects of shape and color."
...
`r func("corrgram::corrgram()")` is closest to @Friendly:02:corrgram, in that it
...
\ixon{scatterplot matrix!generalized}
...
[@FriendlyMeyer:2016:DDAR] for mosaic plots and matrices.
...
`r func("vcd::pairs()")` produces all pairwise mosaic plots, as shown in
...
#| fig-height: 9
#| out-width: "90%"
#| fig-cap: "Mosaic pairs plot for the combinations of species, sex and island. Diagonal plots show the marginal frequency of each variable by the width of each rectangle. Off-diagonal mosaic plots subdivide by the conditional frequency of the second variable, shown numerically in the tiles. The tiles are shaded according to the departures from independence."
library(vcd)
...
The pairwise relations of {species, sex} and {island, sex} are quite balanced.
Row 2 and column 2 show that sex is nearly exactly
proportional among species and islands, indicating independence,
$\text{sex} \perp \{\text{species}, \text{island}\}$.
But for {species, island}, the shading patterns in cells (1,3) and (3,1) of @fig-peng-mosaic show
a strong geographical association: The distribution of
species varies across island, because on each island one or more species
did not occur.  More importantly,
...
triangle. @fig-peng-ggpairs1 includes `sex` (but not `island`) to illustrate the
combinations. Penguin `species` could have been used as another variable, but
it is more naturally represented within the panels as a stratifier.

<!-- fig.code: R/penguin/peng-ggally.R -->
...
#| out-width: "90%"
#| fig-cap: "Basic `ggpairs()` plot of penguin size variables and sex, stratified by species."
knitr::include_graphics("figs/ch04/fig-peng-ggpairs1-1.png")
```
...
penguin size measurements, there are _positive_ correlations within each
species, as we can see in @fig-peng-spm. Yet, in three of these panels,
the overall correlation ignoring species is _negative_. For example, the
...
explored later in @sec-marginal-conditional.
...
built-in functions (of the form `ggally_NAME`), or use your own custom
...
Thus, `r func("ggpairs()")` uses `r func("ggally_cor()")` to print the correlation values
for pairs of continuous variables in the upper triangle, and uses
`r func("ggally_points()")` to plot scatterplots of points in the lower portion.
The diagonal panels as shown as density plots (`r func("ggally_densityDiag()")`)
for continuous variables but as bar plots (`r func("ggally_barDiag()")`) for
discrete factors.
...
scatterplots of continuous variables, there are two method functions which plot
the points and also add a smoother: `ggally_smooth_lm()` and `ggally_smooth_loess()`.
...
To illustrate what is possible, Here is a `GGally` panel function, `my_panel()`, that plots the data points,
...
For this example, I want only simple summaries for the scatterplots, so
I don't want to plot the data points. I do want to add the regression
line and a data ellipse. A revised panel funcion which does this is `my_panel()`.
...
knitr::include_graphics("images/fig-peng-ggpairs7-1.png")
```
...
\ixoff{scatterplot matrix!generalized}
...
or categorical factors, all to be shown along parallel axes. Standard
`ggplot2`-thinking and 2D `geom`s don't work in this wider context.
...
`r func("MASS::parcoord()")`, `r func("GGally::ggparcoord()")` and `r func("PairViz::pcp()")`.
The `r pkg("ggpcp")` version used here is the most general.

-   `r func("pcp_select()")`: _selections_ of the variables to be plotted and their
    horizontal _order_ on parallel axes,
-   `r func("pcp_scale()")`: methods for _scaling_ of the variables to each axis,
-   `r func("pcp_arrange()")`: methods for _breaking ties_ in factor variables to
    space them out.
...
<!-- fig.code: R/penguin/peng-ggpcp.R -->
...
#| out-width: "90%"
#| fig-cap: "Parallel coordinates plot of penguin size variables, together with sex and species."
knitr::include_graphics("images/fig-peng-ggpcp1-1.png")
```
...
#| echo: false
#| fig-cap: "Parallel coordinates plot of penguin size variables, with the levels of species and island reordered."
knitr::include_graphics("images/fig-peng-ggpcp2-1.png")
```
...
## Animated tours {#sec-animated-tours}

In the mid 17$^{th}$  to early 19$^{th}$-century the **Grand Tour**
became a coming-of-age custom for wealthy young Europeans 
...
data space by rotation, @Asimov:85 developed the idea of the data _grand tour_,
a computer method for viewing multivariate statistical data via orthogonal projections onto an animated
sequence of low-dimensional subspaces, like in a movie.
...
space, capturing what it sees from a given location onto the 2D plane of the computer screen.

\ix{PRIM-9}
\ix{grand tour}

More generally, statistical tours are a type of dynamic projections onto orthogonal axes (called a _basis_)
that embed data in a _smaller_
$p$-dimensional space into a $d$-dimensional viewing subspace with $d < p$. Typically, $d=2$, where
the result can displayed as scatterplots, together with vectors representing the projections of the data variables in this space.
But the projected data can be rendered in 1D as densities or histograms, or in more dimensions ($d > 2$) as glyphs,
...
This is nicely illustrated by the cover image (@fig-cover-GEB2) used for Douglas Hofstadter's [-@Hofstadter1979] _Gödel, Escher, Bach_
which shows two 3D solid shapes illuminated by light sources so their shadows form the letters G, B and E projected onto the planes
...
where a 3D dataset is portrayed by the set of _shadows_ of the points on planes formed by pairs of coordinate axes. But the full 3D image is richer.

```{r}
#| label: fig-cover-GEB2
#| echo: false
#| out-width: "40%"
...
knitr::include_graphics("images/Cover-GEB.png")
```
...
perpendicular to $\mathbf{p}$, which is the idea of a shadow. Mathematically, this is calculated as the inner product 
...
To make it easier to relate the points in different views, shapes and colors are assigned so that each point has a unique combination of these attributes.<!-- [^pch] -->

<!--
The unicode characters cause problems in LaTeX with `pdflatex`; they seem to work with `xelatex`.
[^pch]: Plot shapes given by `pch = 15:18` correspond to: 
■ (&#25F9;) filled square (15);
● (&#25CF;) filled circle (16); 
▲ (&#25B2;) filled triangle point-up (17);
◆ (&#25C8;) filled diamond (18).
-->
...
knitr::include_graphics("images/proj-vectors-labels.png")
```
...
In R, we calculate the lengths of _row_ vectors in a projection matrix by transposing and using `r func("matlib::len()")`.
...
To interpret such projections, we want to see **both** the projected data and the signposts (old axes) that tell us where we are
...
onto plots, like @fig-proj-combined (b & c),
to see how the axes in a projection relate to those in the data. 
...
This clever idea of viewing low-dimensional projections of data _together with_ vectors representing the contributions of the
original variables to the dimensions shown in a display is also the basis of **biplot** techniques (@sec-biplot)
we will use in relation to principal components analysis and **canonical discriminant** plots (@sec-candisc)
for MLMs like MANOVA.
...
### Touring methods
The trick of statistical touring methods is to generate a smooth sequence of _interpolated_ projections $\mathbf{P}_{(t)}$
...
This idea, called _projection pursuit_ [@Cook-etal-1995],
\ix{projection pursuit}
works by defining a measure of interestingness of a data projection.
...
#### Penguin tours {.unnumbered}

Penguins are a traveling species. They make yearly travels inland to breeding sites in early spring, repeating the patterns of their ancestors. Near the beginning of summer, adult penguins and their chicks return to the sea and spend the rest of the summer feeding there, but they have different strategies to survive the polar winter [@Black-etal-2018]. The data scientists among them might wonder about the relations among among their cousins of different species on various islands and take a data-centric tour of their 4D measurements of body size with trusty R tools.

<!-- Cite: Black et al Time-lapse imagery of Ade´lie penguins, https://doi.org/10.1371/journal.pone.0193532 DONE -->
...
<!-- **TODO**: ~~This doesn't render well in @fig-peng-tour-demo ~~ -->

<!-- fig.code: R/penguin/peng-tourr.R -->
...
#| echo: false
#| out-width: "100%"
#| fig-cap: "Snapshops of grand tours of the penguin dataset in 2, 3, and 4 dimensions using different `display_*()` methods."
knitr::include_graphics("images/tours/peng-tour-demo-all.png")
```
...
To illustrate better, I'll start with a grand tour designed to explore this 4D space of penguins.
...
A nice feature of `tourr:display_xy()` allows points to be selectively labeled.
I use the three miscreants identified as `noteworthy` in @fig-peng-ggplot-out.

As you watch this pay attention to the separation of the species and any other interesting features. What do you see? 
Watch as Cyrano (case 283) dances around the periphery, never sure of which species to align with.
...
colnames(peng_scaled) <- c("BL", "BD", "FL", "BM")
labs <- rep("", nrow(peng))
labs[noteworthy] <- as.character(noteworthy)
...
differentiates Adélie penguins from the others. In frame (b), the three species
...
<!-- Run peng-tourr.R code again to get labeled points -->
...
#|  - "Clusters"
#|  - "Outliers"
knitr::include_graphics("images/tours/peng-tour-grand-lbls-split/frame_00.png")
knitr::include_graphics("images/tours/peng-tour-grand-lbls-split/frame_10.png")
knitr::include_graphics("images/tours/peng-tour-grand-lbls-split/frame_26.png")
```
...
Let's take the penguins on a _guided tour_, trying to find views that show the greatest separations among their
species; that is, a guided tour, optimizing the `lda_pp()` index of group separation.
...
[**langevitour**](https://logarithmic.net/langevitour/index.html)[@R-langevitour;@Harrison2023] and
`r pkg("liminal", cite = TRUE)` 
...
_all of personality_. These factors are easily remembered by the acronym
**OCEAN**: **O**penness, **C**onscientiousness, **E**xtraversion, **A**greeableness and **N**euroticism.
The dataset, `r dataset("psych::bfi")`, contains data from an online sample of $n=2800$ with 5 items for each scale.
...
and serves as infrastructure for other packages with better graphics. `r pkg("ggraph", cite=TRUE)`
works within the familiar `ggplot2` framework, allowing you to build layered network visualizations using the grammar of graphics. 
The `r package("qgraph")` [@Epskamp-etal-2012] is designed specifically for visualizing statistical models, such as those based on
correlations, partial correlations and other measures of relationship among variables.
...
\ixoff{network diagram}

### Partial correlations {#sec-partial-cor}

\ixon{partial correlation}
...
\ix{graphical gaussian model}
\ix{conditional independence}
...
ignoring other variables, one of partial correlations allows you to visualize the _direct_ relationship between each pair of variables, removing the indirect effects that might be mediated through all other variables. Added-variable plots [@sec-avplots] do something similar for linear models.

For a set of variables $X = \{x_1, x_2, \dots, x_p \}$, the partial correlation
between $x_i$ and $x_j$, controlling for all other variables
...
* If a partial correlation is close to zero, it suggests the relationship between two variables is primarily _mediated_ through other variables.
...
\ix{partial correlation!visualization}
\ixon{partial variables plot}
...
story partial association has to tell. The slope of the regression line in such plots is _exactly_ the partial correlation coefficient
and the shape of the data ellipse gives a visual summary of its strength.

My function `pvPlot()` calculates the partial residuals and then calls
`r func("car::dataEllipse()")` for display. The five most "unusual"
...
<!-- **TODO**: Put `pvPlot` into `heplots` package or `car`. Leave as is for now -->
...
pvPlot(crime.num, vars = c("burglary", "larceny"), 
       id = list(n=5, cex = 1.3),
       ellipse.args = list(col = "red", levels = c(0.68, 0.95)),
       cex.lab = 1.5)
...
#| echo: false
#| out-width: "100%"
#| fig-cap: "Partial variables plots for burglary and larceny (left) and for robbery and auto theft (right) in the network diagram for partial correlations of the crime variables. 'Unusual' points are labeled with their state labels."
knitr::include_graphics("images/crime-pvPlot-1-2.png")
```

These partial variables plots actually _explain_ the details hidden in the numeric summary of the partial correlation or in the network diagram.
...
Arkansas (AK) are smaller on both. This is food for thought.
...
\ixoff{partial variables plot}
\ixoff{partial correlation}

<!---
## Multivariate thinking and visualization

These are just initial notes on a chapter summary, and pointing the way to dimension reduction
...
#### The two curses {.unnumbered}

Multivariate data is often said to suffer from the **curse of dimensionality** [@Bellman1957],
meaning that as the dimensionality of data increases, the volume of the space increases so fast that the available data become sparse, so that the amount of data needed often grows exponentially with the dimensionality.
...
a 2D image decreases rapidly with the number of observations and number of groups represented.
-->

```{r child="summary/Ch04-summary.qmd"}
```

<!-- Exercises moved to online appendix 31-exercises.qmd -->
