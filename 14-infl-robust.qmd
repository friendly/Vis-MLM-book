```{r include=FALSE}
source("R/common.R")
knitr::opts_chunk$set(fig.path = "figs/ch14/")
```
...
# Multivariate Influence and Robust Estimation {#sec-influence-robust}
\ixon{influential observations}
...
groups,
the `r pkg("HLMdiag", cite=TRUE)` provides functions like `r func("hlm_influence()")` and `r func("hlm_augment()")` to generate influence diagnostics and combine them with residuals.
...
\ixmain{influential observations}
...
\ixmain{Cook's distance!multivariate}
...
To illustrate these ideas in this toy example, from @Barrett:2003, considers the simplest case, of one predictor (`x`) and
...
Both of these are also strongly linear with `x` and there is one extreme point (case 9). The data is peculiar,
...
invokes `r func("stats::cooks.distance.lm()")` for the univariate response models, but
invokes `r func("mvinfluence::cooks.distance.mlm()")` for the multivariate model.
...
Let's see how case 9 stands out in the influence plots (@fig-toy-inflplots). It has an extreme hat value. But, because its residual is
...
\ix{influence plot}
In the multivariate version, `r func("mvinfluence::influencePlot.mlm()")` plots the squared studentized residual (denoted `R` in the output) against the hat value; this is referred to as a `type = "stres"` plot.[^cutoffs] 
Case 9 stands out in @fig-toy-inflplot-mlm-stres as wildly influential on the joint regression model. But there's more: The cases in @fig-toy-inflplots with large Cook's D (bubble size)
...
thinking about a problem. Someone with a graphical bent looks at this and thinks,
...
@fig-toy-dfbetas.
The values come from `r func("stats::dfbetas()")` and return the standardized values.
...
\ixoff{Cook's distance}

## Example: NLSY data

The `r dataset("heplots::NLSY")` data introduced in @exm-NLSY-mmra provides a more realistic example
...
#| fig-width: 6
#| fig-height: 6
#| fig-cap: "LR Influence plot for the NLSY data. The dashed lines are contours of increasing Cook's D toward the upper right corner of the plot."
op <- par(mar = c(5,5,1,1))
...
In the influence plot (@fig-peng-inflplot1) the predictor `species` is of course discrete, so there are only
...
<!-- fig.code: R/penguin/peng-manova.R -->
...
The most influential case (283) here is our Chinstrap friend "Cyrano" (see @fig-peng-ggplot-out). Among the others, case 10 is the Adélie bird we labeled "Hook Nose". To better understand _why_ these cases are influential,
...
\ixoff{influential observations}
...
But this advantage comes at the cost of increased computational complexity.[^complexity]

[^complexity]: The tradeoff between simplifying assumptions, which makes computations easier and less restrictive models,
achieved by increased computation lies at the heart of modern statistical methods. In _Computer Age Statistical Inference_,
Efron & Hastie [-@Efron2021] trace this thread in a wide variety ranging from classics in frequentist and Bayesian, through
early computer-age topics to the most recent twenty-first century methods.
...
\ix{weighted least squares}
But because the weights are derived from the data, this must be done
iteratively, requiring more computation.

Several general approaches have been developed for robust multivariate regression. These include M-estimators, S-estimators [@Rousseeuw1984], and MM-estimators [@Yohai1987]. Each approach offers different trade-offs between robustness properties, computational efficiency, and statistical efficiency under ideal conditions.
\ix{robust estimation!S-estimator}
\ix{robust estimation!MM-estimator}
See the [CRAN Task View: Robust Statistical Methods](https://cran.r-project.org/web/views/Robust.html) for an extensive list of
...
The method implemented in the `r func("heplots::robmlm()")` function belongs to the class of **M-estimators**.
...
with the tuning constant $c = 4.685$ for `r func("MASS::psi.bisquare()")`.
...
\ix{iteratively reweighted least squares}
This is implemented in `r func("heplots::robmlm()")`, which generalizes the methods used in `r func("MASS::rlm()")`. ...
...
The `plot()` method for a `"robmlm"` object gives an index plot of the observation weights in the final
...
`r func("heplots::rel_diff()")` calculates the relative difference, $\vert x - y \vert / x$, expressed here as
...
```{r child = "summary/Ch14-summary.qmd"}
```
