...
RStudio does a reasonable job of this in editor windows, using various shades of grey for the text on a colored background
...
Other colors used in the plot are `r colorize("yellow")`, `r colorize("pink")`, `r colorize("grey")` and `r colorize("black")`.
...
Other colors used in the plot are `r colorize_bg("yellow")`, `r colorize_bg("pink")`, `r colorize_bg("grey")` and `r colorize_bg("black")`.
...
colorize2 <- function(text, color) {
  ...
  # For light backgrounds (luminance > 0.5), use dark gray
  # For dark backgrounds (luminance <= 0.5), use light gray
  # Use a more nuanced scale based on luminance
  if (luminance > 0.7) {
    text_color_r <- "gray10"  # Very dark gray for very light backgrounds
    text_color_html <- "#1a1a1a"
  } else if (luminance > 0.5) {
    text_color_r <- "gray20"  # Dark gray for light backgrounds
    text_color_html <- "#333333"
  } else if (luminance > 0.3) {
    text_color_r <- "gray80"  # Light gray for medium backgrounds
    text_color_html <- "#cccccc"
  } else {
    text_color_r <- "gray90"  # Very light gray for dark backgrounds
    text_color_html <- "#e6e6e6"
  }
  
  # Generate output based on format
  if (knitr::is_latex_output()) {
    sprintf("\\colorbox{%s}{\\textcolor{%s}{%s}}", color, text_color_r, text)
  } else if (knitr::is_html_output()) {
...
Other colors used in the plot are `r colorize2("yellow")`, `r colorize2("pink")`, `r colorize2("grey")`, `r colorize2("white")` and `r colorize2("black")`.
...
luminance_df <- data.frame(
  color = test_colors,
  luminance = sapply(test_colors, get_luminance),
  text_color = sapply(test_colors, function(col) {
    lum <- get_luminance(col)
    if (lum > 0.7) "gray10"
    else if (lum > 0.5) "gray20"
    else if (lum > 0.3) "gray80"
    else "gray90"
  })
)
