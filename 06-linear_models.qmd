...
* These models extend directly to the multivariate case of $q > 1$ outcomes, $\mathbf{Y} = (\mathbf{y}_1, \mathbf{y}_2, \dots \mathbf{y}_q)$, and are also fit using `r func("lm()")`.
...
or multinomial regression, fit using `r func("glm()")`.
...
began to layout a general theory of linear models that encompassed _all_ of these separate models,
...
```{r child="child/06-linear-combinations.qmd"}
```
-->

## The General Linear Model {#sec-GLM}

\ixon{general linear model}
...
\ixoff{general linear model}

## Model formulas {#sec-model-formulas}

\ixon{model formulas}
Statistical models in R, such as those fit by `r func("lm()")`, `r func("glm()")` and many other modelling function in R are expressed in a simple notation that was developed by @WilkinsonRogers1973
...
evaluates to the response, such as `log(salary)` or `weight/height^2` which represents the
body mass index.
...
so a model with no intercept $\beta_0 = 0$ can be specified as `y ~ X -1` (or perhaps more
naturally, `y ~ 0 + X`). 
...
by wrapping the term in `r func("I()")`, meaning "identity" or "inhibit". Thus, the model
...
while `r func("poly()")` converts these to orthogonal polynomial scores, which are uncorrelated
(and therefore free from problems of collinearity).

\ixoff{model formulas}
...
This uses
`r func("geom_smooth()")`
and specifies the smoothing model as `method = "lm"` with a `formula`,
...
So a worker with zero years of experience can expect an income of \$29,162 and this should increase by \$1,119
for each additional year. However, it is not so simple to interpret the coefficients when a `r func("poly()")` term
...
The problem is that $x$ = `Experience` in model `workers.mod2` is represented not by the raw values, but rather
by values of $x$ and $x^2$ that have been made to be uncorrelated. If you really want to interpret the coefficient values in terms of years of experience, use the option `raw = TRUE` in `r func("poly()")`:
...
$$
This says that income is predicted to be \$23,068 with no experience, increase initially by \$2,295, but that
...
y & = & \beta_0 + \beta_1 x_1 +  \beta_2 x_2 +  \beta_3 x_1 * x_2 \\
  & = & \beta_0 + (\beta_1 + \beta_3 x_2) x_1 + \beta_2 x_2 \comma \\
\end{eqnarray*}

which means that the coefficient for $x_1$ in the model is not constant for all values of
$x_2$, but rather changes with the value of $x_2$. If $\beta_3 > 0$, the slope for
$x_1$ increases with $x_2$ and vice-versa.

The formula `y ~ A * B` for factors is similar, expanding to `y ~ A + B + A:B`, but the columns in the model matrix
...
using `r func("terms()")` for a `"formula"` object.
...
\ix{model formulas!model matrices}
...
Internally, `r func("lm()")` uses `stats::model.matrix()` and you can use this to explore how factors,
...
taking on values `bc` (blue collar), `wc` (white collar) and `prof` (professional).
...
coded (1, 0) and `type=="wc"`, coded (0, 1). Different coding schemes are described in the following 
section.
...
because, unlike continuous variables, they cannot be entered into the model equation just as they are.
...
Base R provides several coding schemes via assignment to the `r func("contrasts()")` function for
a factor, as in `contrasts(df$Drug) <- ...` one of:

- `contr.treatment()`: Compares each level to a _reference_ level using $k-1$ dummy (0, 1) variables. This is the default, and the reference level is taken as the first (in alphabetic or numerical order). You can change the reference level using `r func("relevel()")` or `r func("reorder()")` for the factor, or simply using `factor(A, levels = ...)`.
...
subset. This is specified as a weighted sum, $L$ of the means $\boldsymbol{\mu}$ with weights $\mathbf{c}$ that
sum to zero,
...
\ixon{contrasts!treatment coding}
...
\ixoff{contrasts!treatment coding}

#### Deviation coding {.unnumbered}

\ixon{contrasts!deviation coding}

Another common coding scheme, useful for unordered factors, is _deviation coding_ given by `contr.sum()`.
...
The parameters estimated with this coding are shown below.
...
\ixoff{contrasts!deviation coding}

#### Helmert contrasts {.unnumbered}

\ix{contrasts!Helmert}
\ix{Helmert contrasts}
\ix{factors!ordered}
...
and they account for non-overlapping portions of the variance for the factor.

\ix{contrasts!orthogonal}
\ix{contrasts!nested dichotomies}
...
of the political spectrum. The following comparisons might be of interest:
