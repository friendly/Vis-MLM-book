...
\subsection{Index entries}\label{index-entries}

Want to index function references automatically.

The functions \texttt{lm()} \index{lm()@\texttt{lm()}} and
\texttt{car::vif()} \index{vif()@\texttt{vif()}} are good test cases.

But \texttt{geom\_smooth()} \index{geom_smooth()@\texttt{geom\_smooth()}}
and \texttt{stat\_ellipse()}
\index{stat_ellipse()@\texttt{stat\_ellipse()}} need to have the
\texttt{\_} escaped inside \texttt{\textbackslash{}texttt\{\}}
