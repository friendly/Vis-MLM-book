...
supplemented or even replaced by suitable _coefficient plots_ providing essentially the same information.
\ix{coefficient plots}
...
library(performance)
library(tidyr)
requireNamespace("spida2", quietly = TRUE)  # GitHub only; used via spida2:: to avoid masking ggplot2::labs()
```
...
Four of these are produced by default, giving rise to the term _regression quartet_ for this collection.
These are:
...
I illustrate some versions from the `r pkg("car", cite=TRUE)` and `r pkg("performance", cite=TRUE)` packages,
part of the `r pkg("easystats", cite=TRUE)` suite of packages.
...
These variables are a bit quirky in they are measured in percents, 0-100, rather than dollars for `income` and
...
confidence intervals is provided by `r func("parameters::model_parameters()")`.
...
with `r func("car::linearHypothesis()")`. This is constructed as a test of a restricted model in which
...
with a **Wald test**[^wald] for difference between these coefficients, expressed as
$\mathcal{H}_0 : \mathbf{C} \boldsymbol{\beta} = 0$, using $\mathbf{C} = (0, -1, 1)$.
The estimated value, -0.053 has a confidence interval [-0.462, 0.356], consistent with 
Duncan's hypothesis.

[^wald]: A Wald test assesses constraints on the parameters in a model.
It is based on the weighted distance between the unrestricted estimate and its hypothesized value under the null hypothesis, where the weight is the precision (inverse variance) of the estimate.
\ix{Wald test}
...
<!-- fig.code: R/Duncan/dunc-conf-ellipse.R -->
...
car::confidenceEllipse(duncan.mod, col = "blue",
  levels = 0.95, dfn = 1,
...
Similar, but nicer-looking diagnostic plots are provided by `r func("performance::check_model()")`
...
<!-- **TODO**:  Already introduced in @exm-prestige Pair this down & integrate with that -->
...
* `education` is measured as the average education of occupational incumbents, in years.
...
From our earlier analyses (@exm-prestige)
...
standard errors, etc. `r func("lmtest::coeftest()")` gives these results in the familiar format for console output.
`r func("broom::tidy()")` places these in a tidy format common to many modeling functions which is useful for
further processing (e.g., comparing models).
```{r coeftest}
...
available from those listed by `r func("get_gof()")`:
...
at the lower left corner. In @fig-modelplot2  I use `r func("scale_y_discrete()")` to reverse the order.
...
the factor. `r func("ggcoef_model()")` plots a single model and `r func("ggcoef_compare()")` plots
...
When we plot this with `r func("ggcoef_model()")`, there are many options to control how variables are
...
effects, they are not large in relation to the scale of `prestige` which ranges from 14.8 to 87.2.

<!-- ## Spread-level plot -->

\ixoff{coefficient plots}

## Added-variable and related plots {#sec-avplots}

\ixon{added-variable plots}
...
However, these simple, _marginal_ scatterplots of a response $y$ against _each_ of several predictors $x_1, x_2, \dots$ can be misleading because each one _ignores_ the other predictors.
...
To see this, consider the dataset `r dataset("matlib::coffee")`, giving measures of `Heart` ($y$), an index of cardiac damage, 
...
[^coffee]: This example was constructed by Georges Monette.
...
#| fig-height: 6
#| out-width: "80%"
#| fig-cap: "Scatterplot matrix showing pairwise relations among `Heart` damage ($y$), `Coffee` consumption ($x_1$) and `Stress` ($x_2$), with linear regression lines and 68% data ellipses for the bivariate relations"
data(coffee, package="matlib")
...
simple marginal scatterplot, designed to visualize directly the partial relation between $y$ and the predictors
$x_1, x_2, \dots$ in a multiple regression model. 

You can think of this as a _magic window_ that hides
...
In effect, this reduces the problem of viewing the complete model
...
In practice, it is unnecessary to run the auxiliary regressions this way [@VellemanWelsh:81].
Both can be calculated using `r func("stats::lsfit()")` roughly as follows:
...
Note that `y` in the code here contains both the current predictor, $\mathbf{x}_i$ and the response $\mathbf{y}$, so the
residuals `resids` have two columns, one for $x_i \,\vert\, \text{others}$
and one for $y \,\vert\, \text{others}$.

#### The `avPlot()` function {.unnumbered}

Added-variable plots are produced using `r func("car::avPlot()")` for one predictor or
`r func("avPlots()")` for any number of model terms. The `id` argument controls
...
\ixoff{added-variable plots}


### Marginal - conditional plots {#sec-marginal-conditional}

\ix{marginal--conditional plots}
...
as in @fig-coffee-avPlots (with a zoomed-out scaling). But here I also overlay the
...
original data points to their positions in the AV plot shows exactly what happens when we condition on
or partial out the other variable.

These _marginal - conditional plots_ are produced by `r func("car::mcPlot()")` (for one regressor)
and `r func("car::mcPlots()")` (for several). The plots for the marginal and conditional relations
...
data ellipse is the _shadow_ (projection) of the ellipsoid for $(\mathbf{y}, \mathbf{x}_1, \mathbf{x}_2)$ on one of the coordinate planes,
whereas the AV plot is a _slice_ through the ellipsoid where either $\mathbf{x}_1$ or $\mathbf{x}_2$
is held constant.
...
See [an animated graphic](https://www.datavis.ca/gallery/animation/duncanAV/) (https://bit.ly/49XDHn0) for an example
...
\ix{component + residual plots}

A related method, the _component + residual plot_ ["C+R plot", also called _partial residual plot_, @Cook:93; @LarsenMcCleary:72] gives a
plot more suited to detecting the need to transform a predictor $\mathbf{x}_i$
to a form $f(\mathbf{x}_i)$ to make its relationship with the response $\mathbf{y}$
more nearly linear.
This plot displays
...
C+R plots are produced by `r func("car::crPlots()")` and `r func("car::crPlot()")`.
...
For example, a quantitative predictor could be fixed at its mean, median or some representative value.
...
* Unusual in the **response** $y$, but typical in the predictor(s), $\mathbf{x}$ --- a badly fitted case with a large absolute residual, but with $x$ not far from the mean, as in @fig-davis-reg2. This case does not do much harm to the fitted model.

* Unusual in the **predictor(s)** $\mathbf{x}$, but typical in $y$ --- an otherwise well-fitted point. This case also does little
harm, and in fact can be considered to improve precision, a "good leverage" point.

* Unusual in **both** $\mathbf{x}$ and $y$ --- This is the case, a "bad leverage" point, revealed in the analysis of Davis's data, @fig-davis-reg1, where the one erroneous point for women was highly influential, pulling the regression line towards it and affecting the estimated coefficient as well as all the fitted values. In addition, subsets of observations can be _jointly_ influential, in that their effects combine, or can mask each other's influence.
...
`r colorize("original data", "blue")` and that for the `r colorize("amended data", "red")`
with the additional point. Note that we use the `levdemo` dataset in `r func("geom_smooth()")` for the
...
$\mu = (30, 30)$, variances = 10, and a correlation $\rho = 0.7$. Then I  add two noteworthy points
...
The Mahalanobis squared distances of these points can be calculated using `r func("heplots::Mahalanobis()")`,
...
Plotting `x1` and `x2` with data ellipses shows the relation of leverage to squared distance from the mean in
@fig-hatvalues-demo1.
...
e_{(-i)} = y_i - \hat{y}_{(-i)} \; .
$$
The (externally) _studentized residual_ is then obtained by dividing $e_{(-i)}$ by its estimated standard error, giving
$$
...
$\mathbf{b} = \widehat{\boldsymbol{\beta}}$ versus their values $\mathbf{b}_{(-i)}$
...
These plots are constructed using `r func("car::influencePlot()")` which also fills the
...
\ixoff{influence plots}

### Influence in added-variable plots

\ixon{added-variable plots!influence}
...
These customized AV plots are produced using calls to `avPlot()` below. 
...
<!-- fig.code: R/Duncan/duncan-av-influence.R -->
...
#| eval: false
#| code-fold: true
#| code-summary: "Code for this figure"
...
\ixoff{added-variable plots!influence}
