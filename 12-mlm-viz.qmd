...
of traditional univariate methods for the same designs. You can carry out multivariate multiple regression,
MANOVA, or indeed, any classical linear model with the standard collection of analysis tools you use for
...
\ix{canonical discriminant analysis}
...
@Huang2019, in a provocative article titled, "MANOVA: A Procedure Whose Time Has Passed?"
criticized these methods as (a) difficult to understand because they are framed in terms of linear combinations of the responses;
...
But that's just what you should expect to happen if you rely on tables of coefficients or ANOVA summary tables, even with significance stars (`*` ... `***`) to help interpret what is important.
(You should be looking at _effect sizes_ and _practical significance_, right?)
...
[^heplots-vignettes]: See the `r pkg("heplots")`  vignettes `vignette("HE_manova", package="heplots")`
...
simple one-way MANOVA design of the dogfood data from @exm-dogfood-data.
...
For more complex models such as MANOVA with multiple factors or multivariate regression with several predictors,
...
\end{aligned}
$${#eq-HE-model}
\ix{sum of squares and products}
...
Anderson wanted to quantify the outward appearance ("morphology": shape, structure, color, pattern, size) of species as a method to study variation within and between such groups. Although Anderson published in the obscure _Bulletin of the American Iris Society_, R. A. Fisher [-@Fisher1936] saw this as a challenge and opportunity to introduce the method now called
discriminant analysis---how to find a weighted composite of variables to best discriminate among existing groups.
@UnwinKleinman2021 recount the history behind this dataset.
...
Anderson's and should not be blamed. After all, if Anderson had given his car to Fisher,
...
variables, using `r func("heplots::glance()")` or `r func("heplots::uniStats()")`. It is of interest that
the univariate $R^2$ values are much larger for the petal variables than the sepal length and width.[^rsq-anova].
For comparison, `r func("heplots::etasq()")` gives the overall $\eta^2$ proportion of variance accounted for in 
all responses.

[^rsq-anova]: Recall that $R^2$ for a linear model is the proportion of variation in the response that is explained by the model, calculated as $R^2 = \text{SS}_H / \text{SS}_T = \text{SS}_H / (\text{SS}_H + \text{SS}_E )$. For a multivariate model, these are obtained from the diagonal elements of $\mathbf{H}$ and $\mathbf{E}$.
...
## Significance scaling vs. effect scaling {#sec-signif-scaling}

\ixon{significance scaling}
...
\ix{visual test of significance}
...
$\mathbf{H}$ matrices are additive.
\ix{orthogonal contrasts}
They fully decompose the general question of differences
...
`r func("car::linearHypothesis()")` is the means for testing these statistically, and `r func("heplot()")`
...
\ixon{canonical discriminant analysis}
...
The canonical $\rho_i^2$, `CanRsq`, gives the R-squared value of fitting the $i$th response canonical variate to the corresponding $i$th canonical variate for the predictors. It is analogous to eta-squared ($\eta^2$ in ANOVA/MANOVA, representing the proportion of variance in the discriminant functions that is accounted for by group membership.
The $\rho_i^2$ are related to the eigenvalues $\lambda_i$ as
\ix{canonical correlation}

$$
\rho_i^2 = \frac{\lambda_i}{1 + \lambda_i} \quad\quad\quad
...
canonical scores.
\ix{canonical structure coefficients}
...
<!-- **TODO**: `var.labels` not a graphical parameter [FIXED] -->
...
\ixoff{canonical discriminant analysis}
...
We can add them here for the term `rate:additive` as shown in @fig-plastic-HE2. This uses `r func("heplots::termMeans()")`
to find the cell means for the combinations of the two factors and then `r func("lines()")` to connect the pairs of points
...
allowing you to see relationships not apparent in all pairwise views.
...
\ixon{multivariate multiple regression}
<!--
...
predictors has a simple interpretation as the correlation between their predicted effects.
Moreover, it is easy to show visual overall tests of _joint_ linear hypotheses for two or more
predictors together.
\ix{joint linear hypothesis}


<!-- **TODO**: Use for an exercise `heplots::Hernior`: Recovery from Elective Herniorrhaphy -> HE_mmra vignette [DONE: exercises/Ch12-exercises.qmd] -->
...
Here I'll continue the analysis of the NLSY data from @exm-NLSY-mmra. In the model `NLSY.mod1`, I used
...
a **robust** method, such as `r func("heplots::roblm()")`. This uses re-weighted least squares to down-weight observations
...
In this model, parents' education, occupation,
and their visits to the schools were highly predictive of student outcomes, but
...
:::
\ixoff{multivariate multiple regression}

## Canonical correlation analysis {#sec-cancor}

\ixon{canonical correlation analysis}
...
For the school data, with $p = 3$ responses and $q = 5$ predictors there are three possible sets of canonical variables. Together these account for 100% of the total linear relations between them. `r func("heplots::cancor()")`
...
accounts for 98.6%, so you might think that is
...
#| fig-height: 8
#| out-width: "65%"
#| fig-cap: "Plots of canonical scores on the first canonical dimension for the `schooldata`, including the influential cases, which stand out as so far from the rest of the observations."
...
<!-- **TODO**: Check out signs of structure coefs from `cancor()`. Would be better to reflect the vectors for `Ycan1`. DONE: fixed in `candisc::heplot.cancor()` by adding a `reflect =` argument. -->
...
:::
\ixoff{canonical correlation analysis}
...
In this example, I'm using the models `Rohwer.sesLo` and `Rohwer.sesHi` fit to each of the groups.
