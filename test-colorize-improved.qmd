...
colorize2 <- function(text, color) {
  ...
  if (luminance > 0.7) {
    text_color <- "gray10"  # Very dark gray for very light backgrounds
  } else if (luminance > 0.5) {
    text_color <- "gray20"  # Dark gray for light backgrounds
  } else if (luminance > 0.3) {
    text_color <- "gray80"  # Light gray for medium backgrounds
  } else {
    text_color <- "gray90"  # Very light gray for dark backgrounds
  }
  
  # Generate output based on format
  if (knitr::is_latex_output()) {
    sprintf("\\colorbox{%s}{\\textcolor{%s}{%s}}", color, text_color, text)
  } else if (knitr::is_html_output()) {
