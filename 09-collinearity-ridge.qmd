...
It describes diagnostic measures to assess these effects (@sec-measure-collin) 
...
Researchers who have studied standard treatments of linear models
...
A more subtle case is the use of _ipsatized_, defined as
...
both 0.15, then geography must be 0.2. Once three of the four scores are known, the last
...
$x_1, x_2, x_3$ for which the pairwise correlations are low, but the multiple correlation is high.
...
so a coefficient that would be significant if the predictors were uncorrelated becomes insignificant
...
Where $X_1$ and $X_2$ are far from their near linear relation in the bottom plane,
...
Then, I define a function to plot the data ellipse (`r func("car::dataEllipse()")`) for each data frame and confidence ellipse
(`r func("car::confidenceEllipse()")`) for the coefficients in the corresponding fitted model. In the plots in @fig-collin-data-beta, I specify the x, y limits for each plot
...
\ixon{VIF}
\ix{collinearity!variance inflation factor}
...
$x^\star_j = x_j  \,|\, \text{others}$. This is, it plots the values of that predictor which uncorrelated with
(or linearly independent of) all the other predictors in the model.
...
\ix{variance inflation factor!generalized}
...
errors. This is what `r func("car::vif()")` calculates for a factor or other term with more than 1 df.
...
We check the variance inflation factors, using `r func("car::vif()")`. We see that most predictors have very high
...
\ix{variance inflation factor!plot}


Beyond the console output from  `r func("car::vif()")`, the `r pkg("easystats")` suite of packages has some useful functions for displaying VIFs in
helpful tables and plots. `r func("performance::check_collinearity()")` calculates VIFs and their standard errors
...
This shows that (a) only the _small_ eigenvalues ($\lambda_k$ in the denominator) contribute to variance inflation, but 
(b) only for those predictors that have large eigenvector coefficients $V_{jk}$ on those small components.
The general principle is that:

> small eigenvalues reflect **redundancy** in the predictors: variables that are measuring nearly the same thing.
...
have common interpretations regardless of the number of predictors.
  ...
  + Thus, these values are perhaps better named **redundancy indices**.
...
These measures can be calculated using `r func("VisCollin::colldiag()")`.
...
\ixon{collinearity!biplot}
\ixon{biplot!collinearity}
...
we can see the main variation in the data and how this is related to the variables.
...
I use `r func("factoextra::fviz_pca_biplot()")` for the biplot in @fig-cars-collin-biplot because I want to illustrate identification of
noteworthy points with `r func("geom_text_repel()")`.
...
It had an 8-cylinder, 455 cu. in, 225 horse-power engine, and was able to go from 0 to 60 mph
in 10 sec.
...
\ixoff{collinearity!biplot}
\ixoff{biplot!collinearity}


## Remedies for collinearity: What can I do? {#sec-remedies}

\ixon{collinearity!remedies for}
...
for visualization methods.
  ...
  Bayesian regression [e.g., @Pesaran2019] can reduce collinearity by imposing shrinkage priors; these incorporate prior information to regularize the model, making it less sensitive to correlated predictors and reducing its _posterior variance_.
...
The effect of centering here is to remove the linear association in what is a purely quadratic relationship.
...
#| fig-height: 5
#| out-width: "90%"
#| fig.cap: "Centering a predictor removes the necessary correlation in a quadratic regression. Left: linear relationship fitting $y$ to the uncentered $x$. Right: fitting to the centered $(x - \\bar{x})$."
r1 <- R[1, 2]
r2 <- R[1, 3]
...
gives the slope at the mean value of $x$.

\ixoff{centering!effect on collinearity}
\ixoff{collinearity!centering}
...
such as response surface models that include all quadratic terms of the predictors and all possible pairwise interactions.
...
<!-- fig.code: R/acetylene-colldiag.R -->
...
degree of freedom, `r func("car::vif()")` calculates GVIFs here.
...
\ixoff{collinearity!remedies for}
\ixoff{collinearity}

## Ridge regression {#sec-ridge}

\ixon{ridge regression}
...
of the tuning parameter, for example using generalized cross validation or bootstrap methods.
...
standardize the columns of $\mathbf{X}$ to unit length, so that $\mathbf{X}\trans \mathbf{X}$ is also a correlation matrix.
...
`r func("glmnet::glmnet()")` also implements a method for multivariate responses with
a `family="mgaussian".
...
\ixon{ridge trace plots!univariate}
...
\ixoff{ridge trace plots!univariate}


## Bivariate ridge trace plots {#sec-ridge-bivar}

\ixon{ridge trace plots!bivariate}
...
\ixoff{ridge trace plots!bivariate}


### Visualizing the bias-variance tradeoff {#sec-bias-variance}
...
In @fig-longley-precision-plot I use the `plot()` method for `"precision"` objects. By default, this plots `norm.beta` against `det`, joins the
points with a smooth spline curve and adds labels for the optimum values according to the different criteria. The points are labelled with
their equivalent degrees of freedom.
...
#| fig-height: 7
#| out-width: "80%"
#| fig-cap: "**Precision plot** showing the tradeoff between bias and precision. Bias increases as we move away from the OLS solution, but precision increases. The slope of the curve indicates the rate at which variance decreases with shrinkage."
...
The  `r func("visCollin::pca()")` method transforms a `"ridge"` object
...
\ixon{ridge regression!biplot}
\ixon{biplot!ridge regression}
...
\ixoff{ridge regression!biplot}
\ixoff{biplot!ridge regression}

\ixoff{ridge regression}


<!-- **TODO**: Consider replacing this with bullet point take-aways. [DONE] -->
