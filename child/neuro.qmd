...
errors $\mathbf{u}_{i}' \sim \mathcal{N}_{p}(\mathbf{0},\boldsymbol{\Sigma})$
...
Calling this function for the model `SC.mlm` produces \figref{fig:SC-cqplot}.
...
re-fit a model to a subset of observations (rather than modifying the data set itself) using
