...
for the new observations. To make this easier, I created a general function `r func("predict_discrim()")`, which joins the measures in the
test dataset with the predicted class for each new observation. In the result, `species` is the predicted class.
[**TODO**: This should be moved to the `candisc` package.]
...
Using this you can simply plot the original Penguin data for two variables and use `r func("geom_text()")` or
...
Data ellipses could then be added added using `r func("car::dataEllipse()")` as follows (not shown)
...
implemented in the `r package("candisc")`. You simply send the `"mlm"` object computed with `lm()`
to `candisc()` and get
...
The significance tests printed are a variety called **step-down tests**

#### Canonical discriminant plot

The `plot()` method for a `"candisc"` object give something similar to what we saw in @fig-peng-LD-biplot.
...
the first canonical dimension largely separates the Gentoos on the left from Adelie and Chinstrap on the right.
...
between the Adelie and Chinstraps, where the Chinstraps tend to have longer bills.
...
Finally, if the focus is on MANOVA, to determine whether and how the penguin species differ
