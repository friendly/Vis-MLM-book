...
Then, in the plotting steps, `geom_tile()` is used to display the predictions in the `pred_grid` dataset, using the penguin colors for the species. Points and other layers use the full `peng` dataset.
```{r peng-ggplot-code}
#| echo: true
#| results: hide
means <- peng |>
...
The groups are nicely separated in the left panel for the bill variables. 