...
#| fig-cap: Influence plot for the demonstration data. The impact of the three unusual points on Cook's D is clearly seen.
...
Let's return to the `Duncan` data used as an example in @sec-example-duncan where a few points stood out as unusual
in the basic diagnostic plots (@fig-duncan-plot-model).
```{r}
