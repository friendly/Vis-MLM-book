...
$$
An oblique projection using all three dimensions is given by $\mathbf{P_2}$ gives a new $\mathbf{Y}$:
$$
 \mathbf{P_2} =
\begin{bmatrix} 
    0.71 & -0.42 \\ 
...
But, if we are traveling in the projection space of $\mathbf{Y}$, we need to have some signposts to tell us how the new dimensions relate to those
of $\mathbf{X}$. The answer is provided simply by plotting the rows of $\mathbf{X}$ as vectors, as shown in @fig-proj-vectors.
In these plots, each row of $\mathbf{P}_1$ and $\mathbf{P}_2$ appears as a vector and shows the contribution each of 
...
$\mathbf{x}_3$ makes no contribution, and appears at the origin. In $\mathbf{P}_2$, $\mathbf{x}_1$ and $\mathbf{x}_2$ make the same contribution
to $\mathbf{y}_1$, but $\mathbf{x}_3$ has no contribution to the horizontal axis.
...
In practice, we can overlay the variable vectors in plots like @fig-proj-combined (b)
...
smoother _geodesic_ path, and this is what the grand tour does. The grand tour algorithm is described
