...
$$
An oblique projection using all three dimensions is given by $\mathbf{P_2}$ below. This produces gives a new 2D view in $\mathbf{Y}$:
$$
 \mathbf{P_2} =
\begin{bmatrix} 
    0.71 & -0.42 \\ 
...
In practice, we want to see **both** the projected data and the signposts that tell us where we are
in relation to the original variables.
we can overlay the variable vectors in plots like @fig-proj-combined (b)
...
#| echo: false
#| out-width: "50%"
#| fig-cap: "The projection of the data"
