...
Mathematicaly, a projection of an $(n \times p)$ data matrix $\mathbf{X}$ representing $n$ observations in $p$
dimensions onto a $d$-dimensional viewing space $\mathbf{Y}$ is represented by a $p \times d$ projection matrix $\mathbf{P}$
...
giving a path of views $\mathbf{Y}_{(t)} = \mathbf{X} \mathbf{P}_{(t)}$ that can be animated in successive frames.
