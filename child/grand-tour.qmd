...
$\mathbf{x}^\textsf{T} \mathbf{p} = x_1 p_1 + x_2 p_2$ and suitably normalized to give the correct length. ...
...
$||\mathbf{x}|| = (\mathbf{x}^\textsf{T} \mathbf{x})^{1/2} = \sqrt{\Sigma x_i^2}$, the Euclidean distance of the tip of the vector from the origin.
...
and
[**langevitour**](https://logarithmic.net/langevitour/index.html) [@R-langevitour]
