...
data from a known model $y = 2 x_1 + 2 x_2 + \epsilon$ with $\epsilon ~ \mathcal{N} (0, 100)$
...
Large VIF$_j$ indicate predictor coefficients whose estimation 
is degraded due to large $R^2_{j \,|\, \text{others}}$.
To go further, we need to determine: 
